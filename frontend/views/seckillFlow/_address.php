...
<div id="addresslist">
    <?php foreach ($address as $k => $v): ?>
        ...
        <label for="address_<?php echo $v['id'] ?>"  class="address_item <?php echo $select_address['id'] == $v['id'] ? 'shopFlBox_address' : 'shopFlBox_address_1'; ?>" >
            ...
            <span class="fr">
                <?php echo CHtml::link(Yii::t('address', '修改地址'), array('/member/address/update/', 'id' => $v['id'], 'turnback' => 1, 'croute' => urlencode($this->createUrl('/' . $this->getRoute(), array('goods_id' => $this->getParam('goods_id')))))); ?>
                <?php
                echo CHtml::link(Yii::t('address', '删除'), array('/member/address/delete/',
                    'id' => $v['id'], 'turnbackByUrl' => 1,
                    'turnbackUrl' => urlencode($this->createUrl('/' . $this->getRoute(), array('goods_select' => 0)))), array('onclick' => 'return confirm(' . Yii::t('address', '确定要删除吗？') . ')'));
                ?>
            </span>
        </label>
    <?php endforeach; ?>
