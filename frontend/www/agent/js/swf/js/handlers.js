...
function uploadSuccess(file, serverData) {
	...
	try {
//		alert("fai:"+serverData.indexOf("fai"));
//		alert("suc:"+serverData.indexOf("suc"));
		if (serverData.indexOf("suc")=='0') {//如果提示成功
			//alert(serverData);
			//var fileinfo = serverData.substring(5).split(",");
			var progress = new FileProgress(file, this.customSettings.progressTarget);
			progress.setComplete();
//			var host = "http://gtimg.gatewang.com";
			//progress.setText("<a href=\"http://gt.gatewang.com/"+fileinfo[0]+fileinfo[1]+"\" target=\"_blank\">"+fileinfo[1]+"</a>");
			var status = "恭喜你，文件上传成功！ <br />";
//			addImage(window.location.protocol+"//"+window.location.host+"/uploads/tmp/"+serverData.substring(4,36)+file.type);//文件名称
			var endnum = serverData.indexOf("|");
			
			var content = serverData.substring(4,endnum);
			var contents = content.split("->,");
			var url = contents[2]+contents[0];										//图片上传之后路径
			var name = file.name;													//文件名称
			var randnum = contents[1];												//保存图片信息的缓存主键
			var path = contents[3];													//保存图片的物理路径
			
//			var host = serverData.substring(45,endnum);
//			var url = host+"/tmp/"+serverData.substring(4,36)+file.type;			//图片上传之后的路径
//			var name = file.name;													//文件名称
//			var randnum = serverData.substring(41,45);								//保存图片信息的缓存主键
			showImage(url,name,randnum,path);
//			var classify = serverData.substring(46,endnum);
//			showImage(url,name,randnum,classify);
			//addImage("thumbnail.php?id=" + serverData.substring(7));
			//status += "大小："+parseFloat(parseInt(fileinfo[2])/1024).toFixed(2)+ "K <br />";
			//status += "地址:http://gt.gatewang.com/"+fileinfo[0]+fileinfo[1];  //这里需要自己根据自己网站实际情况做下修改
			progress.setStatus(status);
			progress.toggleCancel(false);
			//document.getElementById('imglist').innerHTML += '[img]http://http://gt.gatewang.com/'+fileinfo[0]+fileinfo[1]+'[/img]<br/>';
			//getimgtip();
		}else{
			error_img_num++;		//上传失败的文件
			var progress = new FileProgress(file, this.customSettings.progressTarget);
			progress.setError();
			var endnum = serverData.indexOf("|");
	...
	try {
		...
		alert(serverData);
		if (serverData.substring(0, 7) === "FILEID:") {		//这个需要thumbnail.php文件和upload.php文件的配合
			addImage("thumbnail.php?id=" + serverData.substring(7));
...
function showImage(src,name,num,path){
	var img = "<li onclick='choose(this)'><a href='javascript:;'><img src='"+src+"' id='' height='100px' width='100px'></a><input type='hidden' value='"+name+"|"+num+"|"+path+"' /></li>";
	//$('#thumbnails').find("ul").append(img);
...
function queueComplete(numFilesUploaded) {
	...
	if (error_img_num == numFilesUploaded){
		img_html = "没有文件被上传";
	}else{
		var success_num = numFilesUploaded - error_img_num;
		var img_html = success_num + " 个文件上传成功.";
		img_html += error_img_num == 0 ? "" : error_img_num + " 个文件上传失败."
	}
	status.innerHTML = img_html;
}