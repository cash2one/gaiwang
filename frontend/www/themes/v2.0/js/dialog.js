...
function Dialog(content, options) {  
    ...
    /* 对话框的布局及标题内容。*/
    var barHtml = !options.showTitle ? '' : '<div class="bar"><span class="title">' + options.title + '</span><a class="closeDialog">' + options.closeText + '</a></div>';
    var dialog = $('<div id="' + options.id + '" class="dialog '+options.className+'">' + barHtml + '<div class="Dcontent"></div></div>').hide();
    $('body').append(dialog);
    ...
    var init = function() { /* 是否需要初始化背景遮罩层 */
        ...
        /* 绑定一些相关事件。 */
        dialog.find('.closeDialog').bind('click', this.close);
