...
function toCart(){
   ...
   $(document).find('.Carfly').live('click', function() {
		  ...
		  function(){
			  $( this).hide().remove();
			  var num=$('.back—top .num,.mycart #main_top_cart_count');
...
function  backTop() {
 ...
 $(".back—top .backTop,.backtop .backTop,.backtopMap .backTop").click(function(){
     $('body,html').stop().animate({scrollTop:0},500);
     return false;
 });
 
 if ($.browser.msie && ($.browser.version == "6.0") && !$.support.style) {
	  var tobblck = $(".back—top,.backtop,.backtopMap");
	  function ie6(){
		  var w = $(window).width();
		  var h = $(window).height();
	...
	/*ie6滚动*/
	 if ($.browser.msie && ($.browser.version == "6.0") && !$.support.style) {
	  ...
	  function ie6(){
		  ...
		  var i=h-(ht-1000);
		  tobblck.stop().animate({ top: scrollTop + i });
		  document.getElementById("fade").style.height=ht+scrollTop + i;
...
$('.productList li').hover(function(e) {
$(this).addClass('curr');
},function(){
$(this).removeClass('curr');
});
...
/*商品分类*/
 ...
 { /*加盟商服务项目归类介绍*/
	 ...
	 $('#sdTabMenu .sdTabMain ul').imgChange({ thumbObj: '#sdTabMenu .sdTabMenu li', curClass: 'curr', effect:'fade', speed:1, changeTime: 4000 })
	 $('#hotelDetailTab .Htab ').imgChange({ thumbObj: '#hotelDetailTab .hotelTabmenu a', curClass: 'curr', effect:'fade', speed:1, changeTime:6000000 })
	} 
 ...
 function hoverChange(){
	  ...
	  /*查看收起房型信息*/
	   var $$thisa = $('body').find('.room');
		...
		function () {
			var gbName = '收起';
			$(this).parent('.w140').parent('li').parent('.room').find('.hotelIntroTxt').css("display","block");
			$(this).parent('.w140').parent('li').parent('.room').parent('ul').removeClass("room_off");
			$(this).html(gbName)
		},
		function () {
			var zkName = '查看';
			$(this).parent('.w140').parent('li').parent('.room').find('.hotelIntroTxt').css("display","none");
			$(this).parent('.w140').parent('li').parent('.room').parent('ul').addClass("room_off");
			$(this).html(zkName)
		});
