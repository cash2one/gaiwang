...
<head>
    ...
    <script type="text/javascript">
        ...
        var rowCount = '5';
        var addRow = function () {
            if ($('#dataBody').find('tr').length > 4) {
                artDialog.alert("<?php echo Yii::t('sellerDesign','最多只能添加五张图片'); ?>", null, "warning-red");
                return;
            }
            ...
            html += ' <a href="javascript:moveUp(\'' + rowId + '\')" class="px up" title="<?php echo Yii::t('design','向上')?>"></a>';
            html += ' <a href="javascript:moveDown(\'' + rowId + '\')" class="px dow" title="<?php echo Yii::t('design','向下')?>"></a>';
            html += '</td>';
            html += '<td>';
...
<div class="arbox">
    ...
    <div class="con">
        <table class="table_ar" width="100%" cellspacing="0" cellpadding="0" border="0">
            ...
            <tbody id="dataBody">
            <?php 
                if (isset($design->tmpData[DesignFormat::TMP_V20_MAIN_AD]['Imgs'])): 
                   ?>
                <?php foreach ($design->tmpData[DesignFormat::TMP_V20_MAIN_AD]['Imgs'] as $k=>$v):?>
                    <?php $uid = 'row_' . $k ?>
                    <tr id="<?php echo $uid ?>">
                        <td>
                            <?php
                        ...
                        <td><input type="button" class="button_grays"
                                   onclick="doDeleteRow('<?php echo $k ?>')" value="<?php echo Yii::t('sellerDesign','删除'); ?>"></td>
                    </tr>
                <?php endforeach; ?>
            <?php endif; ?>
            </tbody>
    ...
    <div class="msg msg-info" style="width: <?php echo Yii::t('sellerDesign','480')?>; float: left"> <?php echo Yii::t('sellerDesign','图片不能超过5张，请上传 "340x242" 大小的图片。'); ?>
