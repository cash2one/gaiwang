...
<table width="100%" cellspacing="0" cellpadding="0" border="0" class="mt15 sellerT3">
    <tr>
        ...
        <th width="10%" class="bgBlack"><?php echo Yii::t('sellerOrder', '零售价'); ?></th>
    ...
    <?php foreach ($orders as $k => $v): ?>
        <tr>
            <td colspan="8" align="left" valign="middle" class="bgE5">
                <?php if (!$v->is_read): ?>
    ...
    $(".agreerefund").click(function() {
        ...
        art.dialog({
            ...
            ok: function() {
                $.ajax({
                    type: "POST",
                    url: "<?php echo Yii::app()->createAbsoluteUrl('/seller/order/refundNew') ?>",
                    data: {
...
<script>
    ...
    /*
     ...
     $.ajax({
     type: "POST",
     url: "<?php echo Yii::app()->createAbsoluteUrl('/seller/order/signReturnNew') ?>",
     data: {
     "YII_CSRF_TOKEN": "<?php echo Yii::app()->request->csrfToken ?>",
     "orderId": orderId
     },
