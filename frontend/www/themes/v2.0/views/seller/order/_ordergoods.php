    ...
    <tr  class="bgF4">
        ...
        <?php if(($rowSpan && $flag) || $flag):  //和并列 || 显示 ?>
            <td  align="center" valign="middle" <?php echo $rowSpan ?>>
                <p class="red">
                    ...
                    foreach ($order->orderGoods as $v){
                        ...
                        $allPrice += $v['unit_price']*$v['quantity'];
                    }
                    echo HtmlHelper::formatPrice($allGai + $order->freight); 
                    ?>
                </p>
                <p class="gray">（<?php echo Yii::t('sellerOrder','含运费'); ?>：
                    <?php echo HtmlHelper::formatPrice($order->freight); ?>）</p>
            </td>
            <td align="center" valign="middle"  >
                <?php echo HtmlHelper::formatPrice($allPrice + $order->freight);  ?>
                <p class="gray">（<?php echo Yii::t('sellerOrder','含运费'); ?>：
                    <?php echo HtmlHelper::formatPrice($order->freight); ?>）</p>
            </td>
            <td  align="center" valign="middle" class="controlList" <?php echo $rowSpan ?>>
                ...
                <?php endif; ?>
                    <?php if($order->isNew == true){
                        echo $order->exchangeTypeName.Order::exchangeStatus($order->exchangeStatus);
                     }else{ ?>
                    <?php if($order->exchangeType == Order::EXCHANGE_TYPE_RETURN):?>
                        <?php echo $order::refundStatus($order->exchangeStatus) ?>
                    <?php endif ?>
                    <?php if($order->exchangeType == Order::EXCHANGE_TYPE_REFUND): ?>
                        <?php echo $order::returnStatus($order->exchangeStatus) ?>
                    <?php endif ?>
                    <?php } ?>
            ...
            <td  align="center" valign="middle" class="controlList" <?php echo $rowSpan ?>>
                <p><?php echo CHtml::link(Yii::t('sellerOrder','订单详情'),
                ...
                <p><?php echo CHtml::link(Yii::t('sellerOrder','关闭交易'),'#',array('class'=>'closeOrder','data-code'=>$order->code)); ?></p>
                <?php endif; ?>
                ...
                <?php echo CHtml::link(Yii::t('sellerOrder','备货'),'#',array('class'=>'stockup','data-code'=>$order->code)); ?>
                <?php endif; ?>
                ...
                <p><?php echo CHtml::link(Yii::t('sellerOrder','发货'),
                ...
                <p> <?php echo CHtml::link(Yii::t('sellerOrder','签收退货'),$this->createAbsoluteUrl('/seller/order/signReturnNew/',array('code' => $order->code)),array('class'=>'agreeReturn','data-code'=>$order->id,'onclick'=>'return ConfirmDelete('.$order->backMoney.')')); ?></p>
                <?php endif; ?>
                 <?php if($order->refund_status==Order::REFUND_STATUS_PENDING):?>
                    <p><?php echo CHtml::link(Yii::t('sellerOrder','同意退款'),'#',array('class'=>'agreerefund','data-code'=>$order->code,'money'=>$order->backMoney,'data-agree'=>Order::REFUND_STATUS_SUCCESS)); ?></p>
                    <p><?php echo CHtml::link(Yii::t('sellerOrder','不同意退款'),'#',array('class'=>'disagreerefund','data-code'=>$order->code,'data-disagree'=>Order::REFUND_STATUS_FAILURE)); ?></p>
                 <?php endif;?>
            </td>
        <?php endif; ?>
