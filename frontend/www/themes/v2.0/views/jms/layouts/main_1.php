...
<body class="offline offline_detail">
<!-- 头部start -->
    ...
    <div class="navbar-inner">
        <dl>
            ...
            <?php foreach ($this->categorys as $category):?>
                <dd><a <?php if(isset($_GET['city']) && $_GET['city'] == $category['id']){ echo "class='selected'"; } ?> href="<?php echo $this->createAbsoluteUrl('list',array('cat'=>$category['id'],'city'=>$this->city)) ?>"><i class="icon icon-<?php echo empty($category['bussbgclass']) ? 'food' : $category['bussbgclass']; ?>"></i><span><?php echo Yii::t('jms',$category['name'])?></span></a></dd>
            <?php endforeach;?>
        </dl>
    </div>
</div>
...
<script>
    var url = "<?php echo $this->createAbsoluteUrl('list',array('city'=>$city)) ?>";
    $(function(){
        $('#search_btn').click(function(){
            window.location = url+'/keyword/'+$('#search_value').val();
        });
