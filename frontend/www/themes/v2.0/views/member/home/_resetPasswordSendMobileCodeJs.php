...
<script type="text/javascript">
    ...
    function callPhone(btnId,mobileId){
        ...
        if($(btnId).attr('data-status')==0){
            return false;
        }else{
            $(btnId).attr('data-status', '0');
            $(".callInfo").html("<?php echo Yii::t('memberHome', '正在请求……'); ?>");
            $.ajax({
                type: "POST",
     ...
     */
    function startTimer2(btnId, totalTime,mobileId,infoId) {
        var itv = null;
        itv = setInterval(function () {
            totalTime--;
            $(btnId).val('<?php echo Yii::t('memberHome', '重新发送'); ?>(' + totalTime + ')');
            if($(".callInfo").length==0){
                $(btnId).removeClass('btn-send').addClass('btn-sendt');
                $(infoId).html('<a style="color:#c83253" href="javascript:void(0);" onclick="callPhone(\''+btnId+'\',\''+mobileId+'\')" class="callInfo" >没收到短信？'+totalTime+'秒后点此获取语音验证码</a>');
            }else{
                if(totalTime>0){
                    $(btnId).removeClass('btn-send').addClass('btn-sendt');
                    $('.callInfo').html('没收到短信？'+totalTime+'秒后点此获取语音验证码');
                }else{
                    $(btnId).removeClass('btn-sendt').addClass('btn-send');
                    $('.callInfo').html('没收到短信？点此获取语音验证码');
                }
            }
            if (totalTime <= 0) {
                clearInterval(itv);
                itv = null;
                $(btnId).attr("data-status", "1");
     ...
     */
    function sendMobileCode(btnId, mobileId,infoId) {
        $(btnId).click(function () {
            ...
            $(btnId).val("<?php echo Yii::t('memberHome', '发送中……'); ?>");

            setTimeout(function () {
                if($(mobileId).next('.errorMessage:visible').text().length>0){
                    $(btnId).val("<?php echo Yii::t('memberHome', '获取验证码'); ?>");
                    return false;
                }  
                $.ajax({
                    type: "POST",
                    ...
                    success: function (msg) {
                        if (msg === 'success') {
                            <?php
                             echo Tool::getConfig('smsapi','ytxVoiceVerify')==SmsApiConfigForm::VOICE_VERIFY_YES ? 'startTimer2(btnId, 60, mobileId,infoId);':'startTimer(btnId,60);';
                            ?>
                        } else {
                            $(btnId).val('<?php echo Yii::t('memberHome', '发送失败,请重试'); ?>').attr('data-status', 1);
                        }
                    }
                });
            }, 3000);
