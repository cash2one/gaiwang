...
<script type="text/javascript">
    ...
    function callPhone(btnId,mobileId){
        ...
        if($(btnId).find('b').attr('data-status')==0){
            return false;
        }else{
            $(btnId).find('b').attr('data-status', '0');
    ...
    function startTimer2(btnId, totalTime, mobileId) {
        ...
        itv = setInterval(function () {
            totalTime--;
            $(btnId).find('b').html('<?php echo Yii::t('memberHome', '重新发送'); ?>(' + totalTime + ')');
            ...
            if (totalTime <= 0) {
                clearInterval(itv);
                itv = null;
                $(btnId).find('b').attr("data-status", "1");
                $(btnId).find('b').html("<?php echo Yii::t('memberHome', '再次发送'); ?>");
    ...
    /**
     * 更换验证码清空文本框
     * @param btnId 获取更新验证码按钮id
     * @param verifyCode 验证码的inpute id
     */
    function clearCode(btnId,verifyCode){
     ...
     */
    function sendMobileCode(btnId, mobileId, verifyCode) {
        $(btnId).click(function () {
            $("#Member_verifyCode").blur();
            $("#Member_mobile_vcode_em_").remove();
            var mobile = $(mobileId).val();
            ...
            var rs ='';
             $.ajax({
                    type: "POST",
                    url: "<?php echo Yii::app()->createUrl('/member/home/getCaptcha'); ?>",
           ...
           if(code != rs){ 
            ...
            $(btnId).find('b').attr('data-status', '0').html("<?php echo Yii::t('memberHome', '发送中……'); ?>");

            setTimeout(function () {
                if($(mobileId).next('.errorMessage:visible').text().length>0){
                    $(btnId).find('b').attr('data-status', '1').html("<?php echo Yii::t('memberHome', '获取验证码'); ?>");
                    return false;
                }
...
//                }
                $.ajax({
                    ...
                    data: {"YII_CSRF_TOKEN": "<?php echo Yii::app()->request->csrfToken; ?>", "mobile": mobile,'code':code},
                    success: function (msg) {
                        if (msg === 'success') {
                            <?php
                            ...
                            $(btnId).find('b').html('<?php echo Yii::t('memberHome', '发送失败,请重试'); ?>').attr('data-status', 1);
                        }
                    }
                });
            }, 3000);
            ...
            $.ajax({
                ...
                success: function (msg) {
                    if (msg === 'success') {
                        ...
                        $(btnId).find('b').html('<?php echo Yii::t('memberHome','发送失败'); ?>').attr('data-status', 1);
                    }
                }
            });
            return false;
