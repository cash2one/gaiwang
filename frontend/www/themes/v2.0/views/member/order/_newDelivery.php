...
<div class="step-description">
    ...
    <div class="operation">
		...
		<a href="#" class="apply-btn yc-btn delaySign"><?php echo Yii::t('memberOrder','延长签收') ?></a>
		<span class="cover-icon yc-ico">
			<span>
				<?php echo Yii::t('memberOrder','（每次可延长签收时间1天）您还可以延长签收时间{t}次',array('{t}'=>$model->delay_sign_count)) ?>
			</span>
		</span>
		<?php endif; ?>
    </div>
...
$(document).ready(function(e) {
    //延迟签收
	$(".step-description .operation .yc-ico").hover(function(){
		$(this).find("span").show();
	},function(){
		$(this).find("span").hide();
	});
    $(".delaySign").click(function(){
        $.post("<?php echo $this->createAbsoluteUrl('order/delaySign') ?>",{'code': "<?php echo $this->getParam('code') ?>", 'YII_CSRF_TOKEN': '<?php echo Yii::app()->request->csrfToken; ?>'},
            function(data) {
                if (data) {
                    alert("操作成功");
