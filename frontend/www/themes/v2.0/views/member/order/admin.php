...
<div class="main-contain">
  <ul class="order-nav clearfix">
      ...
      <li>
          <a href="<?php
                        echo $this->createAbsoluteUrl('order/admin', array(
                            ...
                            'on' => 'pay'))
                        ?>" class="<?php if($this->getParam('on') == 'pay'){ echo 'active';}?>">
              <span><?php echo Yii::t('memberOrder', '待付款'); ?></span>
              <em class="tm-h"><?php echo $waitPayNum;?></em>
              <i class="interval"></i>
          </a>
      </li>
      <li>
          <a href="<?php
                        echo $this->createAbsoluteUrl('order/admin', array(
                            ...
                            'on' => 'send'))
                        ?>" class="<?php if($this->getParam('on') == 'send'){ echo 'active';}?>">
              <span><?php echo Yii::t('memberOrder', '待发货'); ?></span>
              <em class="tm-h"><?php echo $waitSendNum;?></em>
              <i class="interval"></i>
          </a>
      </li>
      <li>
          <a href="<?php
                        echo $this->createAbsoluteUrl('order/admin', array(
                            ...
                            'on' => 'wait'))
                        ?>" class="<?php if($this->getParam('on') == 'wait'){ echo 'active';}?>">
              <span><?php echo Yii::t('memberOrder', '待收货'); ?></span>
              <em class="tm-h"><?php echo $waitReceiveNum; ?></em>
              <i class="interval"></i>
          </a>
      </li>
      <li>
          <a href="<?php
                        echo $this->createAbsoluteUrl('order/admin', array(
                            ...
                            'on' => 'comment'))
                        ?>" class="<?php if($this->getParam('on') == 'comment'){ echo 'active';}?>">
              <span><?php echo Yii::t('memberOrder', '待评价'); ?></span>
              <em class="tm-h"><?php echo $waitCommentNum; ?></em>
              <i class="interval"></i>
          </a>
      </li>
	  ...
	  <li>
		  <a href="<?php
		  echo $this->createAbsoluteUrl('order/admin', array(
			  'Order[life]' => $model::LIFE_RECYCLE,
			  'on' => 'recycle'))
		  ?>" class="<?php if($this->getParam('on') == 'recycle'){ echo 'active';}?>">
			  <span><?php echo Yii::t('memberOrder', '订单回收站'); ?></span>
			  <i class="interval"></i>
		  </a>
	  </li>
...
/*上一页*/
function pagePrev(){
   	window.location.href = '<?php echo $this->createAbsoluteUrl('order/admin', array('Order[code]'=>$code, 'page'=>$prevPage));?>';
}

/*下一页*/
function pageNext(){
	window.location.href = '<?php echo $this->createAbsoluteUrl('order/admin', array('Order[code]'=>$code, 'page'=>$nextPage));?>';
}
...
//还原与彻底删除订单
$(".recycleOrder,.deleteOrder,.deleteOrderFirst").click(function(){
    var order_code = $(this).attr("data_code");
    if($(this).hasClass('recycleOrder')){
        var msg = '<?php echo Yii::t('memberOrder', '你确定要还原订单吗？'); ?>';
        var life = "<?php echo Order::LIFE_DEFAULT ?>";
    }else if($(this).hasClass('deleteOrder')){
         msg = '<span style="padding:0px;line-height:14px;"><?php echo Yii::t('memberOrder', '永久删除后，您将无法再查看该订单，也无法进行投诉维权，请谨慎操作！'); ?></span>';
         life = "<?php echo Order::LIFE_DELETE ?>";
    }else{
        msg = '<span style="padding:0px;line-height:14px;"><?php echo Yii::t('memberOrder', '删除后，您可在订单回收站找回，或永久删除。'); ?></span>';
        life = "<?php echo Order::LIFE_RECYCLE ?>";
    }
    layer.confirm(msg, {
        btn: ['<?php echo Yii::t('memberOrder', '确定'); ?>','<?php echo Yii::t('memberOrder', '取消'); ?>'], //按钮
        shade: false,
//       area: ['350px', '350px'],
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo $this->createAbsoluteUrl('order/recycle') ?>",
            data: {
                "YII_CSRF_TOKEN": "<?php echo Yii::app()->request->csrfToken ?>",
                "code": order_code,
                'life':life
