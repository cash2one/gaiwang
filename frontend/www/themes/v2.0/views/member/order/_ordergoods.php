...
<?php foreach ($orderGoods as $k=>$v):?>
    <tr class="order-bd">
      ...
      <td class="price"><?php echo HtmlHelper::formatPrice($v['unit_price'])/*Common::rateConvert($v['unit_price'])*/;?></td>
      ...
      <?php
        ...
        if($order->life==$order::LIFE_RECYCLE){
            echo CHtml::link(Yii::t('memberOrder', '还原'),  'javascript:void(0);', array('class' => 'pay-btn recycleOrder', 'data_code' => $order->code));
            echo CHtml::link(Yii::t('memberOrder', '永久删除'),  'javascript:void(0);', array('class' => 'pay-btn deleteOrder', 'data_code' => $order->code));
        }
        if($order->status!=$order::STATUS_NEW && $order->life==Order::LIFE_DEFAULT){
            echo CHtml::link(Yii::t('memberOrder', '删除'),  'javascript:void(0);', array('class' => 'pay-btn deleteOrderFirst', 'data_code' => $order->code));
        }
      ?>
      <?php if($blank){
