...
if(!empty($model->username)){
	...
	//为了统一和头部一样,如果登录后修改了用户名,则显示头部的用户名 (暂不清楚这个是不是企业名称,先注释)
	/*if($this->getUser()->id && !$this->getUser()->isGuest){
		$userName = $this->getUser()->name;
	}*/
}else if(!empty($model->mobile)){
    $userName = substr($model->mobile, 0, 3).'****'.substr($model->mobile, -4);
}else{
	$userName = $model->gai_number;
}
...
<div class="member-section">
        <div class="basic-info">
            <div class="info-details clearfix">
                <span class="avatar">
                    ...
                    <?php else: ?>
                        <?php echo CHtml::image( Yii::app()->theme->baseUrl. '/images/bgs/default-avatar.png', '头像', array('width' => 102, 'height' => 102)); ?>
                    <?php endif; ?>
                </span>
                <span class="info-01">
                    <b class="name"><?php echo $userName; ?></b>
        ...
        <?php }else{?>
        <div class="no-order" style="display:block;"><?php echo Yii::t('member','还没有订单记录');?></div>
        <?php }?>
        ...
        <?php }else{?>
        <div class="no-collection"><?php echo Yii::t('member','还没收藏过商品');?></div>
        <?php }?>
    </div>
