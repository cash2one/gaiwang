...
?>
<style>
.phone_tips{ padding-left:auto;}
</style>
<script type="text/javascript">
    //ajax 获取语音验证码
//    var itv = null;
    function callPhone(btnId,mobileId){
        var new_mobile = $(mobileId).val();
        if (new_mobile && !new_mobile.match(/^1[34578]{1}\d{9}$/)) {
        ...
        if($(btnId).find('span').attr('data-status')==0){
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo Yii::app()->createUrl('/active/auction/getMobileVerifyCall'); ?>",
                data: {"YII_CSRF_TOKEN": "<?php echo Yii::app()->request->csrfToken; ?>", "new_mobile": new_mobile},
                success: function (msg) {
                    if (msg === 'success') {
                        startTimer2(btnId, 60, new_mobile);
    ...
    function startTimer2(btnId, totalTime, mobileId) {
        ...
        var itv = setInterval(function () {
            ...
            if($(".phone_tips .getCallPhone").length==0){
                ...
                $(btnId).after('<div class="phone_tips" ><a href="javascript:void(0);" onclick="callPhone(\''+btnId+'\',\''+mobileId+'\')" class="getCallPhone red" >没收到短信？'+totalTime+'秒后点此获取语音验证码</a></div>');
            }else{
                if(totalTime>0){
    ...
    function sendMobileCode(btnId, mobileId,fn) {
        $(btnId).click(function () {
            ...
            if (!new_mobile) {
                alert('<?php echo Yii::t('memberMember', '请输入手机号码！') ?>');
               // $(this).after('<span class="revise-message" id="Member_mobile_vcode_em_" style=""><?php echo Yii::t('memberMember', '您未设置手机号码') ?></span>');
                return false;
            }
            if (!new_mobile.match(/(^1[34578]{1}\d{9}$)|(^852\d{8}$)/)) {
                alert('<?php echo Yii::t('memberMember', '请输入正确的手机号码！'); ?>');
...
//            startTimer(btnId,59);
            ...
            setTimeout(function () {
                ...
                $.ajax({
                    type: "POST",
                    url: "<?php echo Yii::app()->createUrl('active/auction/getMobileVerifyCode'); ?>",
                    data: {"YII_CSRF_TOKEN": "<?php echo Yii::app()->request->csrfToken; ?>", "new_mobile": new_mobile},
                    success: function (msg) {
                        if (msg !== 'success') {
