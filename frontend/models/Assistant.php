...
class Assistant extends CActiveRecord
{
    ...
    {
        $arr = array(
            self::STATUS_NO =>Yii::t('assistant', '未启用'),
            self::STATUS_YES =>Yii::t('assistant', '启用'),
        );
    ...
    {
        $arr = array(
            self::SEX_MALE =>Yii::t('assistant', '男'),
            self::SEX_FEMALE =>Yii::t('assistant', '女'),
        );
        return isset($arr[$k]) ? $arr[$k] : $arr;
    }
	...
	public function rules()
	{
		return array(
            ...
            array('avatar', 'file', 'types' => 'jpg,gif,png', 'maxSize' => 1024 * 1024 * 5,
                'tooLarge' => Yii::t('member', '文件大于5M，上传失败！请上传小于5M的文件！'), 'allowEmpty' => true),
            array('username', 'match', 'pattern' => '/^[\x{4e00}-\x{9fa5}A-Za-z0-9_]+$/u',
                'message' => Yii::t('assistant', '公司名称 只能由中文、英文、数字及下划线组成')),
            array('confirmPassword','compare','compareAttribute'=>'password','on'=>'insert,update'),

            array('mobile', 'comext.validators.isMobile', 'errMsg' => Yii::t('assistant', '请输入正确的手机号码')),
	...
	{
		return array(
			'id' => Yii::t('assistant','主键'),
			'username' => Yii::t('assistant','用户名'),
			'password' => Yii::t('assistant','密码'),
			'salt' => Yii::t('assistant','密钥'),
			'member_id' => Yii::t('assistant','所属会员'),
			'real_name' => Yii::t('assistant','真实姓名'),
			'avatar' => Yii::t('assistant','头像'),
			'sex' => Yii::t('assistant','性别'),
			'mobile' => Yii::t('assistant','手机号码'),
			'email' => Yii::t('assistant','邮箱'),
			'status' => Yii::t('assistant','状态'),
			'logins' => Yii::t('assistant','登录次数'),
			'description' => Yii::t('assistant','说明'),
			'sort' => Yii::t('assistant','排序'),
			'create_time' => Yii::t('assistant','创建时间'),
			'update_time' => Yii::t('assistant','更新时间'),
			'confirmPassword' => Yii::t('assistant','确认密码'),
			'oldPassword' => Yii::t('assistant','旧密码'),
			'newPassword' => Yii::t('assistant','新密码'),
		);
	}
