 ...
 */
Yii::import ( 'application.modules.seller.controllers.FranchiseeController' );
class FranchiseeUploadController extends FranchiseeController{
	const FILE_NAME = "cache_imgpaths";			//缓存文件名称
	const FILE_UPLOAD_NAME = "Filedata";		//文件上传名称
	const FILE_TMP_PATH = "tmp";				//文件上传临时目录
	const FILE_PATH = "tmp";				//文件上传临时目录
	const NAMERANDMIN = 1000;					//名称随机尾数最小值
	const NAEMRANDMAX = 9999;					//名称随机尾数最大值
	
	public $file_path = 'files';
	
	function init(){
		$this->file_path = empty($_REQUEST['folder_name'])?'files':$_REQUEST['folder_name'];
		parent::init();
	}
	...
	public function actionIndex()
	{
		...
		$this->render('index',array(
			...
			'img_model'=>$img_model,
			'folder_name'=>empty($_REQUEST['folder_name'])?'files':$_REQUEST['folder_name'],
		));
	}
	...
	public function actionUpload(){
		...
		$foldername = empty($_REQUEST['folder_name'])?'files':$_REQUEST['folder_name'];


		$fileName = md5(uniqid('',true).rand(self::NAMERANDMIN, self::NAEMRANDMAX)).".".$file_extension;

		$path = $foldername.DS.date('Y').DS.date('m').DS.date('d').DS;
		$save_path = str_replace('\\','/',Yii::getPathOfAlias('uploads').DS.$path);
        ...
        self::HandleSuc($randnum."->,".IMG_DOMAIN."/".str_replace('\\', '/', $path)."$fileName|");


        //保存入库
        $pic_model = new FranchiseePicture();
        $pic_model->franchisee_id = $this->curr_franchisee_id;
        $pic_model->path =  str_replace('\\', '/', $path).$fileName;
        $pic_model->save();
        //保存操作记录
        $this->_saveLog(SellerLog::logCateFranchiseeUploadUpload,SellerLog::logTypeInsert);
        exit(0);
	...
	public function actionSure(){
		...
		foreach ($imgs as $img){		//每个循环对象里面有两个值，图片url和id
			...
			$newpath = $imgAreaData['path'].$path.$filename;				//	D:/wamp/www/source/uploads/123.jpg		D:/wamp/www/uploads/2013/12/12/123.jpg
	 ...
	 * @param unknown_type $ids   用,分割的id列表
	 */
	function actionDel($ids){
		$pics = FranchiseePicture::model()->findAll("franchisee_id={$this->curr_franchisee_id} AND id IN ($ids)");
		
		
		//删除文件
		if(FranchiseePicture::model()->deleteAll("franchisee_id={$this->curr_franchisee_id} AND id IN ($ids)")){
			foreach($pics as $pic){
                $filePath = Yii::getPathOfAlias('uploads').DS.$pic['path'];
                UploadedFile::delete($filePath);
			}
			
			//保存操作记录
        	$this->_saveLog(SellerLog::logCateFranchiseeUploadDel,SellerLog::logTypeDel,$this->curr_franchisee_id);
	...
	public static function getImgArea($key){
		$imgAreaArr = array(
			1 => array(
				'area' => IMG_DOMAIN,
				'path' => Yii::getPathOfAlias('uploads'),
			),
			2 => array(
				'area' => ATTR_DOMAIN,
				'path' => Yii::getPathOfAlias('att'),
			),
		);
