        ...
        <?php if(($rowSpan && $flag) || $flag):  //和并列 || 显示 ?>
            ...
            <td  align="center" valign="middle" class="controlList" <?php echo $rowSpan ?>>
                ...
                <?php if($order->status==$order::STATUS_NEW): //新订单才显示先关支付、物流状态 ?>
                    ...
                    <?php if($order->refund_status!=$order::REFUND_STATUS_NONE): ?>
                        <?php echo $order::refundStatus($order->refund_status).Yii::t('sellerOrder','退款') ?>
                    <?php endif ?>
            ...
            <td  align="center" valign="middle" class="controlList" <?php echo $rowSpan ?>>
                ...
                <?php elseif ($order->return_status==Order::RETURN_STATUS_AGREE):?>
                    <p> <?php echo CHtml::link(Yii::t('sellerOrder','签收退货'),$this->createAbsoluteUrl('/seller/order/signReturn/',array('code' => $order->code)),array('class'=>'agreeReturn','data-code'=>$order->id,'onclick'=>'return ConfirmDelete('.$order->backMoney.')')); ?></p>
                <?php endif; ?>
                <?php if($order->refund_status==Order::REFUND_STATUS_PENDING):?>
                    <p><?php echo CHtml::link(Yii::t('sellerOrder','同意退款'),'#',array('class'=>'agreerefund','data-code'=>$order->code,'data-agree'=>Order::REFUND_STATUS_SUCCESS)); ?></p>
