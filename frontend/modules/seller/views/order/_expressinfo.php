 <h3 class="mt15 tableTitle"><?php echo Yii::t('sellerOrder','物流信息');?>&nbsp;
     <?php if(($orderDetail['delivery_status'] == Order::DELIVERY_STATUS_SEND) && $orderDetail['status'] == Order::STATUS_NEW):?>
         <a onclick="changeExpress()" class="sellerBtn05" href="javascript:void(0)"><span><?php echo Yii::t('sellerOrder', '修改'); ?></span></a>
     <?php endif;?>
 </h3>
            <table width="100%" cellspacing="0" cellpadding="0" border="0" class="mt10 sellerT3">
                <tbody><tr>
                        <th width="10%"><?php echo Yii::t('sellerOrder','配送状态');?></th>
                        <td width="90%">
                            <?php echo Order::deliveryStatus($orderDetail['delivery_status']);?>
                        </td>
                    </tr>
                    <tr>
                        <th><?php echo Yii::t('sellerOrder','快递公司');?></th>
                       <td><?php echo Yii::t('express',$orderDetail['express'])?></td>
                    </tr>
                    <tr>
                        <th><?php echo Yii::t('sellerOrder','快递单号');?></th>
                        <td><?php echo $orderDetail['shipping_code']?></td>
                    </tr>
                    <tr>
                        <th><?php echo Yii::t('sellerOrder','物流动态');?></th>
                       <td id="express_orstatus">
                          <?php echo Yii::t('sellerOrder','暂无物流动态');?> 
                        </td>
                    </tr>
                </tbody>
            </table>
            ...
            <?php if(!empty($orderDetail['shipping_code'])):?>
                <script>
                $("#express_orstatus").html('<?php echo Yii::t('sellerOrder','正在查询物流信息.....');?>');
