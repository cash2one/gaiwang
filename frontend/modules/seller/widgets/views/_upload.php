...
echo CHtml::button($btn_value, array(
        ...
        "class" => $btn_class,
		'id'=>'_imgBut_'.$toid,
        "onclick" => "_fileUpload('" . Yii::app()->createUrl('/seller/upload/index', array('height' => $height, 'width' => $width, 'img_format' => $img_format)) . "','" . Yii::app()->createUrl('/seller/upload/sure', array('imgarea' => $img_area, 'foldername' => $folder_name, 'isdate' => $isdate)) . "',$num,'" .$toid . "','" . Yii::app()->request->csrfToken . "','".session_id()."')",
    )
)
?>
<?php if ($model): ?>
