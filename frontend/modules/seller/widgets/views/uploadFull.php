...
<?php 
	echo CHtml::link(Yii::t('seller','编辑图片空间'),'javascript:;',array(
			...
			"onclick"=>"_fileUpload('".Yii::app()->createUrl('/seller/franchiseeUpload/index',array('height'=>$height,'width'=>$width,'img_format'=>$img_format,'folder_name'=>$folder_name))."','".Yii::app()->createUrl('/seller/upload/sure',array('imgarea'=>$img_area,'foldername'=>$folder_name,'isdate'=>$isdate))."',$num,'".get_class($model)."_".$attribute."','".Yii::app()->request->csrfToken."','".session_id()."')",
		));
?>
