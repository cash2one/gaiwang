...
<div class="com-box">
    ...
    <div class="sign-plan clearfix">
        ...
        <?php  foreach(OfflineSignStoreExtend::getAuditStatus111() as $key => $value):?>
            <p
...
<!--                        <a href="--><?php //echo $this->createUrl('offlineSignContract/printView',array('contractId'=>$storeInfo['cid'],'name'=>$storeInfo['b_name']))?><!--">打印合同</a>-->
                            <?php if(!empty($model->offline_sign_contract_id)):?>
                        <a href="<?php echo $this->createUrl('offlineSignContract/printView',array('contractId'=>$model->offline_sign_contract_id,'extendId'=>$model->id))?>">打印合同</a>
                            <?php endif;?>
                        </p>
                    <?php endif;?>
                    <?php
                        $count -= 1;
                        $mark =false;
                        if($model->audit_status == OfflineSignStoreExtend::AUDIT_STATUS_SUB_ELECTR ){
                            $count==0 && $mark = true;
                        }
                    ...
                    ?>
                        <p class="why">
                            ...
                            <a href="<?php echo $this->createUrl('offlineSignStoreExtend/uploadContract',array('storeExtendId'=>$model->id))?>">上传合同</a>
                        </p>
                    <?php } ?>
                </td>
            </tr>
