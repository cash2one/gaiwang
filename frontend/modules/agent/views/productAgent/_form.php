<?php 
	...
	$baseUrl = AGENT_DOMAIN.'/agent';//js ,css,image存放路径
...
function selectBizName(){
	art.dialog.open("<?php echo $this->createUrl('productAgent/showBiz')?>",{
		title: "<?php echo Yii::t('Product','选择加盟商')?>",
		lock: true,
		width: 880,
		height: 610,
		init:function(){},
...
var GWIncomeRate = parseFloat(<?php echo ProductAgent::getGWIncomeRate()?>);
var BuyerIncomeRate = parseFloat(<?php echo ProductAgent::getBuyerIncomeRate()?>);

//判断百分比分配
var computeReturn = function () {
    if (!$('#ProductAgent_back_rate').val()) {		//CommissionRate返佣率
        return;
    }
    if (!$('#ProductAgent_price').val()) {					//零售价
        return;
    }
    if (!$('#ProductAgent_gt_rate').val()) {			//MachineIncome盖机收益
        return;
    }

    var back_rate = parseFloat($('#ProductAgent_back_rate').val());
    var price = parseFloat($('#ProductAgent_price').val());
    var machineIncome = parseFloat($('#ProductAgent_gt_rate').val())

    if (back_rate < 0 || back_rate > 100) {
        return;
    }
    if (machineIncome < 0 || machineIncome > 100) {
        return;
    }
	$.get('<?php echo $this->createUrl('ajaxReturnScore')?>', {back_rate:back_rate,price:price,machineIncome:machineIncome},function(data){
		$("#ReturnScore").text(data);
		});
}
</script>
...
<div class="ctx">
    ...
    <div class="ctxTable">
		...
		<table width="100%" border="0" cellspacing="1" cellpadding="0" class="inputTable">
	        <tbody>
	            ...
	            <tr>
	                ...
	                <td>
	                    <span id="ReturnScore" style="color:Red; font-weight:bold;"><?php echo $model->return_score?></span>
	                </td>
	            </tr>
	            <tr>
	            ...
	            <tr>
	                ...
	                <td>
						...
						<?php echo CHtml::link(Yii::t('Public','选择加盟商'),'javascript:selectBizName()',array('class'=>'button_05'))?>
	            ...
	            <tr>
                    ...
                    $this->widget('ext.editor.WDueditor', array(
                        'model' => $model,
                        'base_url'=>'http://agent.'.SHORT_DOMAIN,
                        'attribute' => 'content',
	    ...
	    <div class="align-center">
        		...
        		<?php echo CHtml::link(Yii::t('Public','返回'),Yii::app()->createUrl('productAgent/index'),array('class'=>'button_04'))?>
	    </div>
	<?php $this->endWidget();?>      
</div>    
</div>
