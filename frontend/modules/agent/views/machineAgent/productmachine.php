...
<script type="text/javascript">
	 ...
	 */
	function  addMachine(){
		var url = createUrl("<?php echo $this->createUrl('productAgent/addMachine')?>",{"id":<?php echo $model->product_id?>});
		art.dialog.open(url,{
...
$('#machine2ProductAgent-form').submit(function(){
	$('#machine2ProductAgent-grid').yiiGridView('update', {
		data: $(this).serialize()
	});
	$('#addid').val('');
	return false;
...
<div class="ctx">
	<div class="optPanel">
		<div class="toolbar img01"><?php echo Yii::t('Machine','绑定盖机')?>
			...
			<?php echo CHtml::link(Yii::t('Machine','添加盖机'),$this->createUrl('productAgent/addMachine',array('id'=>$model->product_id)),array('class'=>'button_05 floatRight','onclick'=>'return addMachine()'))?>
		</div>
	</div>
	<?php 
		$form = $this->beginWidget('CActiveForm',array(
			'id'=>'machine2ProductAgent-form',
		));
