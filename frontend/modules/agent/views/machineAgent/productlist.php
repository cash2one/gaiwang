...
<script type="text/javascript">
    ...
    function removeChoose() {
        ...
        var myUrl = createUrl("<?php echo $this->createUrl('machineAgent/productRemove') ?>", {"id":<?php echo $model->id ?>, "product_id": idData});
        art.dialog({
            title: "<?php echo Yii::t('Machine', '解除绑定') ?>",
            icon: 'question',
            content: "<?php echo Yii::t('Machine', '确认解除选中广告在本盖机的绑定') ?>?",
            lock: true,
            okVal: '<?php echo Yii::t('Public', '确定') ?>',
            ok: function() {
                location.href = myUrl;
...
//        			$.fn.yiiListView.update("product-agent-list");
...
//                        }else if(choose_id == 'ad_product'){
//                            product_num = product_num - num;
//                            var tab_html = "产品管理("+product_num+") ";
//                        }
...
//        	});
     ...
     */
    function addAdvert() {
        var url = createUrl("<?php echo $this->createUrl('machineAgent/productAdd') ?>", {"id":<?php echo $model->id ?>});
        art.dialog.open(url, {
            title: "<?php echo Yii::t('Machine', '搜索商品') ?>",
            lock: true,
            height: 630,
            width: 980,
            ...
            ok: function() {
                ...
                var addAdvertListUrl = createUrl("<?php echo $this->createUrl('machineAgent/productCreate') ?>", {"product_id": id, "id":<?php echo $model->id ?>});
                ...
                jQuery.ajax({
                    ...
                    success: function(data) {
                        ...
                        var choose_id = $('.tab1_01').find('.tabhover').id;
                        if (choose_id == 'ad_coupon') {
                            coupon_num = coupon_num + num;
                            var tab_html = "优惠劵（" + coupon_num + "）";
                        } else if (choose_id == 'ad_sign') {
                ...
                <tr class="caption">
                    <td colspan="2">
                        <div class="tab1 fl">
                            ...
                            ?>
                            <?php echo CHtml::link(Yii::t('Machine', '优惠劵') . "（" . $coupon . "）", $this->createUrl('machineAgent/AdvertList', array('id' => $model->id, 'adtype' => MachineAdvertAgent::ADVERT_TYPE_COUPON)), array('class' => $classCoupon, 'id' => 'ad_coupon')) ?>
                    ...
                    <td colspan="2">
                        <div class="panel">
                            <div class="adClassList">
                                <ul>
                                    <?php
                                    $allClass = $productModel->category_pid == "" ? "selected" : "";
                                    $allStyle = $productModel->category_pid == "" ? "" : "background:url(/images/home.gif) no-repeat;";
                                    ?>
                                    <li><li><?php echo CHtml::link(Yii::t('Public', '所有分类'), $this->createUrl('machineAgent/productList', array('id' => $model->id, 'category_pid' => '', 'category_id' => '')), array('class' => 'tab $allClass', 'style' => "$allStyle")) ?></li></li>
                                    <?php
                                    $pclass = "";
                                    $pstyle = "background:url(/images/home.gif) no-repeat;";
                                    foreach ($typeData as $key => $val):
                                        if ($productModel->category_pid != "") {
                                            if ($val['id'] == $productModel->category_pid) {
                                                $pclass = "selected";
                                                $pstyle = "";
                                            } else {
                                                $pclass = "";
                                        ...
                                        ?>
                                        <li>
                                            <?php echo CHtml::link($val['name'], $this->createUrl('machineAgent/productList', array('id' => $model->id, 'category_pid' => $val['id'])), array('class' => "tab $pclass", 'style' => "$pstyle")) ?>
                                        </li>
                                    <?php endforeach; ?>
                                </ul>
                            </div>
                                ...
                                <ul>
                                    ...
                                    foreach ($typeChild as $keyChile => $valChile):
                                        if ($productModel->category_id != "") {
                                            if ($valChile['id'] == $productModel->category_id) {
                                                $cclass = "selected";
                                            } else {
                                                $cclass = "";
                                            }
                                        ...
                                        <li class="tab <?php echo $cclass ?>" onclick="checkLi(this)">
                                            <a href="<?php echo $this->createUrl('machineAgent/productList', array('id' => $model->id, 'category_pid' => $valChile['pid'], 'category_id' => $valChile['id'])) ?>">
                        ...
                        <div class="headerBar">
                            ...
                            <?php if ($adtype != MachineAdvertAgent::ADVERT_TYPE_VEDIO) {//视频没有排序?>
                                <?php echo CHtml::link(Yii::t('Public', '排序'), $this->createUrl('machineAgent/productsort', array('id' => $model->id, 'adtype' => $adtype)), array('class' => 'button_04')) ?>
                            <?php } ?>
                        </div>
                        ...
                        <div class="adListItem">
                            <ul class="ad-list">
                                ...
                                $this->widget('application.modules.agent.widgets.ListView', array(
                                    'dataProvider' => $productModel->searBingProduct(),
                                    'itemView' => 'productview',
                                    'id' => 'product-agent-list'
                                ));
                                ?>
                            </ul>
                        </div>
