...
<head lang="en">
	...
	<link rel="stylesheet" href="<?php echo CSS_DOMAIN ?>/global.css"/>
    <link rel="stylesheet" href="<?php echo CSS_DOMAIN ?>/member.css"/>
    <style>
        .mbRcontent{
            ...
            <div class="mbDate1_c">
                ...
                <ul style="width:230px; border:1px solid #ccc; float:right; margin:50px 46px 0 0; padding:10px;<?php if(!$enterprise->service_id && !isset($this->model->referrals->gai_number)) echo ';display:none;' ?>">
                    <?php if ($enterprise->service_id): ?>
                        <li>
                            招商人员服务编号：<?php echo $enterprise->service_id ?>
                        </li>
                    <?php endif; ?>
                    <?php if (isset($this->model->referrals->gai_number)): ?>
                        <li>
                            商铺推荐号：<?php echo $this->model->referrals->gai_number ?>
                        </li>
                    <?php endif; ?>
                </ul>
				</div>
				<h3 style="line-height:30px; text-align:center;">我的网络店铺签约资质</h3>
