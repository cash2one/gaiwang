...
/**
    ...
    public function checkCart(Array $cartInfo)
    {
        ...
        foreach ($cartInfo as $k => $v) {
            ...
            //检查是否是积分+现金订单,特殊商品,红包商品
                 ...
                 */
                if(isset($v2['seckill_seting_id']) && $v2['seckill_seting_id']>0)  throw new CHttpException(403, '商品在wap端不可购买，请到pc端购买');
                if ($v2['jf_pay_ratio'] > 0 && $v2['jf_pay_ratio'] < 100)
                   throw new CHttpException(403, '商品信息有误！，现金积分商品不可购买，请到pc端购买');
             if ($v2['quantity'] > $v2['stock']) throw new CHttpException(403, Yii::t('orderFlow', '购物车中有商品缺货，请重新确认或下单!'));
             $goodsStock[] = array(
    ...
    private function _createOrder($cartInfo, $address, $info)
    {
		...
		foreach ($cartInfo['cartInfo'] as $k => $v) {
            ...
            if ($goodsOne['jf_pay_ratio'] > 0 && $goodsOne['jf_pay_ratio'] < 100) {
