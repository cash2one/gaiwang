            ...
            <div class="navTab clearfix">
                <div class="tabPages fl clearfix">
                    ...
                    if (!empty($model)):?>
                        ...
                        <?php else: ?>
                            <a href="<?php echo $this->createAbsoluteUrl('search/search', array('p' => $product, 'order' => $order, 'tid' => $tid, 'page' => ($page - 1))); ?>"
                               class="page">上一页</a>
                        <?php endif; ?>
                ...
                <div class="tabFilter fr clearfix">
                    <a class="icoModel iM01 <?php if ($tid == 1): ?> selected <?php endif; ?>" id="showProImg"
                       href="<?php echo $this->createAbsoluteUrl('search/search', array('p' => $product, 'order' => $order, 'tid' => 1)); ?>"></a>
                    <a class="icoModel iM02 <?php if (empty($tid)): ?> selected <?php endif; ?>" id="showProList"
                       href="<?php echo $this->createAbsoluteUrl('search/search', array('p' => $product, 'order' => $order, 'tid' => 0)); ?>"></a>
                    <a class="icoCon" href="#"><i class="icoTriangle"></i></a>
                </div>
            </div>
            <?php $order = $this->getParam('order'); ?>
            <div class="filter clearfix" id="filter">
                <a href="<?php echo $this->createAbsoluteUrl('search/search', array('p' => $product, 'order' => 0, 'tid' => $tid)); ?>" <?php if (empty($order)): ?> class="item selected" <?php else: ?> class="item" <?php endif; ?>>综合</a>
                <a href="<?php echo $this->createAbsoluteUrl('search/search', array('p' => $product, 'order' => 1, 'tid' => $tid)); ?>" <?php if ($order == 1): ?> class="item selected" <?php else: ?> class="item" <?php endif; ?>>销量</a>
                <a href="<?php echo $this->createAbsoluteUrl('search/search', array('p' => $product, 'order' => 3, 'tid' => $tid)); ?>" <?php if ($order == 3): ?> class="item selected" <?php else: ?>
                   class="item" <?php endif; ?>class="item">价格</a>
                <a href="<?php echo $this->createAbsoluteUrl('search/search', array('p' => $product, 'order' => 4, 'tid' => $tid)); ?>" <?php if ($order == 4): ?> class="item selected" <?php else: ?>
