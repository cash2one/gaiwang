...
class ImageTool {
    ...
    public static function resizeImage($path, $maxwidth, $maxheight) {
        //新的图片路径
        $base_path = Yii::getPathOfAlias('upload');
        $path = str_replace('/uploads/', '', $path);
        $image = $base_path . DS . $path;  //图片的地址
        if (!file_exists($image)) {
            return '#';
        }
        $new_file_name = 'uploads' . DS . $maxwidth . 'x' . $maxheight . DS . $path;
        $image_info = pathinfo($new_file_name);
        $new_dirname = $image_info['dirname'];
        if (!is_dir($new_dirname)) {
            Tool::createDir($new_dirname);
        }
        $filename = $base_path . DS . $maxwidth . 'x' . $maxheight . DS . $path;    //新图片地址

        if (file_exists($filename)) {
            $url = IMG_DOMAIN . '/' . $maxwidth . 'x' . $maxheight . '/' . $path;
            $url = str_replace('\\', '/', $url);
            return $url;
        }
        ...
        if (($maxwidth && $pic_width > $maxwidth) || ($maxheight && $pic_height > $maxheight)) {
            ...
            call_user_func($output_fun, $newim, $new_file_name);
