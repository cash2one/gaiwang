...
class Thumber
{
    /** @var bool url 中是否有配置目录key,盖网通默认是没有的，手动指定为 files */
    private $issetConfigKey = true;
    /**
     * @var Config
     */
    protected $config;
    ...
    */
    ...
    public function getSourcefile()
    {
        ...
        $filePath = $this->url->getImagePath($this->issetConfigKey);
    ...
    {
        if(@method_exists($this->getImage(),'save')){
    ...
    protected function process()
    {
        ...
        //var_dump($url,$urlImageName,$newImageName);exit;
        //Keep unique url
        ...
        if(file_exists($cachePath)){
            $this->processed = true;
            $this->thumber = false;
            $this->getThumber($cachePath); //读取缓存文件
            return true;
        }
