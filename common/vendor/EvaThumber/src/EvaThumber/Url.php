...
class Url
{
    ...
    */
    ...
    public function getUrlKey()
    {
        ...
        */
        $urlImagePath = THUMB_SRC;
        $urlImagePathArray = explode('/', ltrim($urlImagePath, '/'));
        return $this->urlKey = $urlImagePathArray[0];
    }
    ...
    public function getUrlImagePath()
    {
        ...
        */
        return $this->urlImagePath = THUMB_SRC;
    }
    ...
    public function getUrlImageName()
    {
        ...
        */
        return $this->urlImageName = basename(THUMB_SRC);
    ...
    /**
     * 获取图片路径
     * @param bool $issetConfigKey 是否存在配置key
     * @return string
     */
    public function getImagePath($issetConfigKey=true)
    {
        $urlImagePath = $this->getUrlImagePath();
        $urlImagePathArray = explode('/', ltrim($urlImagePath, '/'));
        if(count($urlImagePathArray) < 2){
        ...
        if($issetConfigKey){
            return $this->imagePath = '/'.dirname($urlImagePath);
        }else{
            return $this->imagePath = dirname($urlImagePath);
        }
    }
    ...
    public function toString()
    {
        ...
        return $url.'?'.dirname($this->getQuery()).'/'.$this->urlImageName;
    }
    ...
    public function __construct($url = null)
    {
        ...
        if($urlString){
            ...
            //$this->query = null;
            $this->urlPath = THUMB_SRC;
            $this->urlImagePath = THUMB_SRC;
        }
    }
}
