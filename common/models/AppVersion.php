...
class AppVersion extends CActiveRecord {
	 ...
	 * @return array
	 */
    const APP_TYPE_GAME_SWITCH = 0;       //积分兑换开关，用于IOS平台
    ...
    const APP_TYPE_COVER_PAPER = 9;               //盖讯通
    const APP_TYPE_SKU_FRESH_MACHINE_RK3288 = 10;           //SKU生鲜机RK3288主板
    ...
    const APP_TYPE_GAME_AFEIJI = 26;                    //打飞机游戏
    const APP_TYPE_GAME_TANTIAOGONGZHU = 27;            //弹跳公主游戏
    const APP_TYPE_GAME_LVGUANG = 29;            //绿光游戏
    const APP_TYPE_GAME_ZOUGANGSI = 30;          //美女走钢丝游戏
    const APP_TYPE_GAME_HOUSAILEI = 31;            //猴犀利游戏
    const APP_TYPE_GAME_JUMP = 32;                       //超神特工游戏
    const APP_TYPE_GAME_BAOWEISHIFU = 33;            //保卫师傅游戏
    const APP_TYPE_GAME_XIONGHAIZI = 34;            //熊孩子逃学记游戏
    const APP_TYPE_GAME_GOUGOU = 35;            //狗狗别作死游戏
    const APP_TYPE_GAME_SHOURENLAILE = 36;            //兽人来了游戏
    const APP_TYPE_GAME_DAMAOXIAN = 37;            //盖付通大冒险游戏
    const APP_TYPE_GAME_SHENRUDIHOU = 38;            //深入敌后游戏
    const APP_TYPE_GAME_JINJIMOWANG = 39;            //进击魔王
    const APP_TYPE_GAME_DAYUCHIXIAOYU = 40;            //大鱼吃小鱼
    const APP_TYPE_GAME_DALUANDOU = 41;               //盖付通大乱斗
    const APP_TYPE_GAME_ZHAINANYUNVPU=42;            //宅男与女仆
    const APP_TYPE_GAME_TAOCHUDONGWUYUAN=43;        //逃出动物园
	...
	public static function getAppType($key = false) {
		$status = array(
				...
				self::APP_TYPE_SKU_FANHE_MACHINE => 'SKU饭盒机',
                                self::APP_TYPE_SKU_FRESH_MACHINE_RK3288 =>'SKU生鲜机RK3288主板',
                self::APP_TYPE_GT_INVENTORY_APP => '盖网通盘点',
                self::APP_TYPE_COVER_PAPER => '盖讯通',
                ...
                self::APP_TYPE_GAME_AFEIJI => '打飞机游戏',
                self::APP_TYPE_GAME_TANTIAOGONGZHU => '弹跳公主游戏',
				self::APP_TYPE_GAME_LVGUANG => '绿光游戏游戏',
                self::APP_TYPE_GAME_ZOUGANGSI => '美女走钢丝',
                self::APP_TYPE_GAME_JUMP => '超神特工游戏',
				self::APP_TYPE_GAME_HOUSAILEI => '猴犀利游戏',
				self::APP_TYPE_GAME_BAOWEISHIFU => '保卫师傅游戏',
				self::APP_TYPE_GAME_XIONGHAIZI => '熊孩子逃学记游戏',
				self::APP_TYPE_GAME_GOUGOU => '狗狗别作死游戏',
                self::APP_TYPE_GAME_SHOURENLAILE => '兽人来了游戏',
                self::APP_TYPE_GAME_DAMAOXIAN  => '盖付通大冒险游戏',
		self::APP_TYPE_GAME_SHENRUDIHOU  => '深入敌后游戏',
                self::APP_TYPE_GAME_JINJIMOWANG  => '进击魔王游戏',
                self::APP_TYPE_GAME_DAYUCHIXIAOYU  => '大鱼吃小鱼游戏',
                self::APP_TYPE_GAME_DALUANDOU =>'盖付通大乱斗游戏',
		self::APP_TYPE_GAME_ZHAINANYUNVPU=>'宅男与女仆游戏',
		self::APP_TYPE_GAME_TAOCHUDONGWUYUAN=>'逃出动物园游戏'
		);
		if ($key === false)
			return $status;
		return $status[$key];
    ...
    public function rules() {
        return array(
            array('system,system_type,type,app_type, name, version, version_name,package_name, web_log, mobile_log, is_visible, is_published, is_auto_download', 'required'),
            array('is_visible, is_published, is_auto_download', 'numerical', 'integerOnly' => true),
            array('name, url,img_url, version, version_name, package_name,apk_name', 'length', 'max' => 128),
            ...
            array('id, name, url,img_url,ios_img_url, size, version, version_name,package_name, web_log, mobile_log, remark, is_visible, is_published, user_id, create_time, update_time, ip, apk_name, is_auto_download,ios_ipd', 'safe', 'on' => 'search'),
        );
    }
     ...
     * @param int $SystemType 系统类型
     * @return max
     */
    public static function getAppUrl($SystemType,$AppType){
    	$app =Yii::app()->db->createCommand()
    	 ->select('url')
    	 ->from('{{app}}')
    	 ->where('type = :type AND system_type=:stype AND app_type=:atype AND is_visible=:visible AND is_published=:published',
    	    array(':type'=>self::FLAG_TYPE_SOFTWARE,':stype'=>$SystemType,':atype'=>$AppType,':visible'=>self::VISIBLE_YES,':published'=>self::PUBLISHED_YES))
    	 ->order('id DESC')
    	 ->queryScalar();
    	return $app;
     }
