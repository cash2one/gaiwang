...
class GameStore extends CActiveRecord
{
    ...
    public function rules() {
        return array(
            ...
            array('limit_time_hour,limit_time_minute,send_num', 'numerical', 'integerOnly'=>true),
            array('send_num','compare', 'compareValue'=>'1000000', 'operator'=>'<=', 'message'=>Yii::t('send_num','必须小于等于1000000')),
            array('limit_time_hour,send_num', 'compare', 'compareValue'=>'0', 'operator'=>'>=', 'message'=>Yii::t('gameStore', '必须大于等于0')),
            ...
            array('franchise_stores','checkSrore'),
            ...
            array('id, gai_number,store_name, store_phone,store_address, store_status,limit_time_hour,limit_time_minute, create_time, update_time, franchise_stores, send_num', 'safe'),
        );
    }
     ...
     * @param $attribute
     */
    public function checkSrore($attribute){
        if($attribute){
            $rs = Yii::app()->db->createCommand()
                ->select('id')
                ->from('{{game_store}}')
                ->where('franchise_stores = :fs',array(':fs'=>self::FRANCHISE_STORES_IS))
                ->queryScalar();
            if($rs && $this->franchise_stores == self::FRANCHISE_STORES_IS && $this->id != $rs){
                $this->addError($attribute, Yii::t('GameStore', '只能指定一家特殊商品店铺'));
            }
        }
    }
    ...
    public static function  getEnterpriseGaiNumber($str){
        ...
        if(preg_match('/^GW[0-9]{8}$/',$str)){
            $id= Yii::app()->db->createCommand()->select('id')->from('{{member}}')
                ->where('(status=:status_no or status=:status_yes ) and gai_number=:gai_number and enterprise_id != :id',array(':status_no'=>Member::STATUS_NO_ACTIVE,':status_yes'=>Member::STATUS_NORMAL,':gai_number'=>$str,':id' => Member::ENTERPRISE_NO))
                ->queryScalar();
            return $id ? $id : '';
        }else {
            return '';
    ...
    public function search() {
        ...
        $criteria->compare('franchise_stores', $this->franchise_stores, true);
