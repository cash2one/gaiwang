...
class Region extends CActiveRecord {
     ...
     * @author LC
     */
    public $agent_gai_number;
    public $agent_username;
    public $agent_mobile;
    public $agent_old_gai_number;
    ...
    public static function getArea(){
        $models = RegionManage::model()->findAll();
        $areaArr = array();
        foreach ($models as $v){
            $areaArr[$v->id] = $v->name;
        }
     ...
     * @return array
     */
    public static function getFranchiseeArea($agent_id = null){
        try {
            if ($agent_id === null) {
                $agent_id = Yii::app()->user->id;
            }
            $manageId = Yii::app()->db->createCommand()
                ->select('manage_id,tree,parent_id')
                ->from(Region::model()->tableName())
                ->where('member_id = :id', array(':id' => $agent_id))
                ->queryAll();
            $str = '(';
            if (!empty($manageId)) {
                foreach ($manageId as $val) {
                    if(($val['manage_id'] == 0) && ($val['parent_id'] != 1) && ($val['parent_id'] != 1)){
                        $manageSheng = explode('|',$val['tree']);
                        $parent_manage_id = Yii::app()->db->createCommand()
                            ->select('manage_id')
                            ->from(Region::model()->tableName())
                            ->where('id = :id', array(':id' => $manageSheng['1']))
                            ->queryScalar();
                        if(!empty($parent_manage_id))
                            $val['manage_id'] = $parent_manage_id;
                    }
                    $str .= $val['manage_id'] . ',';
                }
                $str = rtrim($str, ',');
            ...
            return !empty($areaArr) ? $areaArr : array();
        }catch(Exception $e){
            echo "<script>alert('foreach有错');</script>";
        }
    }
    /**
     * 根据省份id返回大区id
     * @param $id
     */
    public static function getAreaIdByProvinceId($id){
        $model = Region::model()->findByPk($id);
        if($model){
            return $model->manage_id;
        }else return '';
    }
    ...
    public static function getAreaNameById($id){
        if($id){
            $model = RegionManage::model()->findByPk($id);
            if(!empty($model))
                return $model->name;
            else
                return '';
     ...
     * @param int $id 大区id
     */
    public static function getProvinceByAreaId($id){
        if (empty($id))
            return array();

        $models = self::model()->findAll('manage_id = ' . $id);
        $provinceArr = array();
        foreach ($models as $v) {
     ...
     * @return integer  
     */
    public static function getAreaIdByUserId($userId)
    {
        $criteria=new CDbCriteria;
        $criteria->select = "t.id,t.manage_id";
        $criteria->condition = 't.member_id=:memberId';
        $criteria->params = array(':memberId'=>$userId);
        $res = self::model()->find($criteria);
        return empty($res) ? null : $res->manage_id;
    }
...
//            array('name', 'unique'),  //不同地区下面，可能会有相同的名称
            ...
            array('name,agent_gai_number,agent_username,agent_mobile', 'safe', 'on' => 'search'),
        );
    }
    ...
    {
    	$criteria=new CDbCriteria;
    	$criteria->select = 't.id,t.name,t.depth,m.gai_number as agent_gai_number,m.username as agent_username,m.mobile as agent_mobile';
    	$criteria->join .= 'left join {{offline_role_relation}} o on o.region_id = t.id ';
    	$criteria->join .= 'left join {{member}} m on o.member_id = m.id ';
    	$criteria->addCondition('t.depth>0');
    	$criteria->compare('t.name',$this->name,true);
    	$criteria->compare('m.gai_number',$this->agent_gai_number,true);
    	$criteria->compare('m.mobile',$this->agent_mobile,true);
    	$pagination = array(
    			'pageSize' => 25
    	);
...
//     	self::regionIndexing(); // 生成分类索引
    ...
    public static function getRegionName($id) {
        ...
        static $regions = null;
        if($regions===null){
           $regions = Tool::cache(self::CACHEDIR)->get(self::CK_ALLRegion);
        }
...
//        );
     ...
     * @return array
     */
    public static function getHotelCityPinyin(){
        $city = Tool::cache(self::CACHEDIR)->get('getCityPinyin');
        ...
        $sql = 'SELECT DISTINCT
            ...
            FROM
                gw_hotel AS h
            LEFT JOIN gw_region as r on r.id=h.city_id
            WHERE
                h.`status` = :s ';
        $data = Yii::app()->db->createCommand($sql)->bindValue(':s',Hotel::STATUS_PUBLISH)->queryAll();
        $r = array();
        foreach($data as $k => $v){
            if(empty($v)) continue;
            $r[$k]['city_id'] = $v['city_id'];
            $name = str_replace('市','',$v['name']);
            $r[$k]['name'] = $name;
            $r[$k]['allPy'] = Tool::getPinyin($name,false);
            $r[$k]['head'] = Tool::getPinyin($name);
        }
        Tool::cache(self::CACHEDIR)->set('getCityPinyin', $r);
        return $r;
    }
