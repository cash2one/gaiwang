 ...
 */
class SellerLog extends CActiveRecord {
    ...
    public function attributeLabels() {
        return array(
            'id' => Yii::t('sellerLog','主键'),
            'category_id' => Yii::t('sellerLog','所属分类'),
            'title' => Yii::t('sellerLog','标题'),
            'type_id' => Yii::t('sellerLog','所属类型'),
            'create_time' => Yii::t('sellerLog','创建时间'),
            'source' => Yii::t('sellerLog','操作对象'),
            'source_id' => Yii::t('sellerLog','操作对象ID'),
            'member_id' => Yii::t('sellerLog','所属管理'), //会员id或者店小二id
            'member_name' => Yii::t('sellerLog','会员名称'),
            'ip' => 'IP',
            'is_admin' => Yii::t('sellerLog','是否管理员'),
        );
    }
...
//        $criteria->compare('is_admin', $this->is_admin);
        ...
        if (Yii::app()->user->getState('assistantId')) {
            ...
            if (!empty($assistantIds)){
                $criteria->addCondition('is_admin='.self::ADMIN_NO.' and member_id in('.implode(',',$assistantIds).')','or');
            }
        }
    ...
    public static function create($category_id = 0, $type_id = 0, $source_id = 0, $sub_title = '') {
        ...
        return Yii::app()->db->createCommand()->insert('{{seller_log}}',$logArr);
    }
