...
//注册类型（0默认，1盖网机，2盖网，3手机短信，4手机APP,5路由，6第三方注册）
    ...
    const REG_TYPE_THIRD = 6;
    const REG_TYPE_TOKEN = 7;
    const REG_TYPE_GXAPP = 8;
    const REG_TYPE_SKU = 9 ;         //SKU
    const REG_TYPE_HOTEL = 10 ;      //酒店
    const REG_TYPE_DRUGSTORE = 11 ;  //盖象大药房
    const REG_TYPE_GAME = 12;        //游戏注册
    const REG_TYPE_OFFLINE = 13 ;  //电子化签约审核通过（新商户）
    ...
    public static function registerType($key = null) {
        $arr = array(
            ...
            self::REG_TYPE_MACHINE => Yii::t('member', '盖网机'),
            self::REG_TYPE_WEBSITE => Yii::t('member', '盖网'),
            self::REG_TYPE_MESSAGE => Yii::t('member', '手机短信'),
            self::REG_TYPE_APP => Yii::t('member', '手机APP'),
            self::REG_TYPE_ROUTER => Yii::t('member', '路由'),
            self::REG_TYPE_THIRD=>Yii::t('member', '第三方导入'),
            self::REG_TYPE_TOKEN => Yii::t('member', '盖付通'),
            self::REG_TYPE_GXAPP =>Yii::t('member','盖象APP'),
            self::REG_TYPE_SKU => Yii::t('member','SKU'),
            self::REG_TYPE_HOTEL => Yii::t('member','酒店'),
            self::REG_TYPE_DRUGSTORE => Yii::t('member','盖象大药房'),
            self::REG_TYPE_GAME => Yii::t('member','游戏'),
            self::REG_TYPE_OFFLINE => Yii::t('member','电子化签约'),
        );
        if (is_numeric($key)) {
    ...
    const IDENTITY_TYPE_PASSPORT = 5;
    ...
    public static function identityType($key = null) {
        $arr = array(
            ...
            self::IDENTITY_TYPE_PASSPORT => Yii::t('member', '护照证'),
        );
        if (is_numeric($key)) {
            return isset($arr[$key]) ? $arr[$key] : null;
        } else {
    ...
    public function rules() {
        $rules = array(
            ...
            //修改绑定手机
            array('mobileVerifyCode2,mobile2', 'required', 'on' => 'updateMobile'),
            array('password', 'required', 'message' => Yii::t('member', '登录密码 不可为空白'), 'on' => 'updateMobile'),
//            array('mobileVerifyCode', 'comext.validators.mobileVerifyCode', 'on' => 'updateMobile,mRegister'),
            array('password', 'checkPassword', 'on' => 'updateMobile,updateBindEmail'),
...
//修改头像
            ...
            array('username', 'match', 'pattern' => '/^(?!(\d|GW\d{8,10}))[\x{4e00}-\x{9fa5}A-Za-z0-9_\(\)（）]+$/u', 'on' => 'enterpriseCreate',
                'message' => Yii::t('member', '公司名称 只能由中文、英文、数字及下划线组成,不能是纯数字、GW号')),
...
//注册
            array('verifyCode', 'comext.validators.requiredExt', 'allowEmpty' => !CCaptcha::checkRequirements(),
                'message' => Yii::t('member', '{attribute} 不能为空！'), 'on' => 'register,quickRegister,regEnterprise'),
//            array('verifyCode', 'comext.validators.requiredExt', 'allowEmpty' => !self::showCaptcha(),
//                'message' => Yii::t('member', '{attribute} 不能为空！'), 'on' => 'register'),
            array('verifyCode', 'captcha', 'allowEmpty' => !self::showCaptcha(), 'captchaAction' => 'captcha2', 'on' => 'regEnterprise,register'),
            array('verifyCode', 'captcha', 'allowEmpty' => !CCaptcha::checkRequirements(), 'on' => 'quickRegister'),
            array('password', 'required', 'on' => 'regEnterprise,register,mRegister'),
            array('password,confirmPassword', 'match', 'pattern' => '/([\s]+)/', 'not' => true, 'message' => Yii::t('member', '密码不能有空格!')),
...
//            array('confirmPassword', 'compare', 'compareAttribute' => 'password', 'on' => 'resetPassword'),
            array('tmp_referrals_id', 'comext.validators.isGaiNumber', 'on' => 'register,regEnterprise,quickRegister,upgrade'),
            array('tmp_referrals_id', 'exist', 'attributeName' => 'gai_number', 'className' => 'Member', 'on' => 'register,regEnterprise,quickRegister'),
            //验证手机号码
            array('mobile,mobileVerifyCode', 'required', 'on' => 'regEnterprise,registerStep2,bind,register,mRegister,bindMobile'),
            array('mobileVerifyCode', 'comext.validators.mobileVerifyCode',
                'on' => 'regEnterprise,registerStep2,bind,register,mRegister,bindMobile'),
            array('mobile', 'comext.validators.isMobile', 'errMsg' => Yii::t('member', '请输入正确的手机号码'),
                'on' => 'regEnterprise,resetPassword,resetPassword2,resetPassword3,registerStep2,update_base,register,mRegister,update_enterprise,bindMobile'),
            //快速手机号注册,重置密码
            array('mobile', 'exist', 'attributeName' => 'mobile', 'className' => 'Member', 'on' => 'resetPassword',
...
//            array('password,confirmPassword', 'required', 'on' => 'resetPassword'),
            //后台普通会员添加,前台修改基本信息
            array('mobile,identity_type,identity_number,real_name,sex,identity_type', 'required', 'on' => 'create'),
            array('mobile', 'required', 'on' => 'create_enterprise'),
            array('mobile', 'required', 'on' => 'update_enterprise'),
            //删除状态的时候，手机号码等字段可留空
            array('mobile', 'required', 'on' => 'update_base'),
            //后台企业会员添加
//            array('referrals_id', 'required', 'on' => 'enterpriseCreate,enterpriseUpdate'),
            array('mobile', 'comext.validators.isMobile', 'errMsg' => Yii::t('member', '请输入正确的手机号码'), 'on' => 'enterpriseCreate,enterpriseUpdate,update_base,update,create'),
            //修改二、三级密码
            array('password2,confirmPassword', 'required', 'on' => 'resetPassword2'),
            array('password3,confirmPassword', 'required', 'on' => 'resetPassword3'),
            array('confirmPassword', 'compare', 'compareAttribute' => 'password2', 'message' => '必须与新密码一致！', 'on' => 'resetPassword2'),
//            array('confirmPassword', 'compare', 'compareAttribute' => 'password3', 'message' => '必须与新密码一致！', 'on' => 'resetPassword3'),
            array('password', 'length', 'min' => 6,'on'=>'register,quickRegister,regEnterprise,FindPassCheck,resetPassword'),
            array('password2', 'length', 'max' => 128, 'min' => 6),
            array('password3, confirmPassword', 'length', 'min' => 6),
            array('gai_number', 'unique'),
            array('mobile', 'comext.validators.mobileUnique', 'on' => 'regEnterprise,enterpriseCreate,register,create,update,update_base,enterpriseUpdate,insert,registerStep2,bindMobile'),
            array('email', 'email'),
            array('username,email', 'unique', 'on' => 'enterpriseCreate,register,create,update,update_base,enterpriseUpdate'),
            array('username', 'length', 'max' => 50, 'min' => 3, 'on' => 'enterpriseCreate,register,create,update,update_base,enterpriseUpdate,quickRegister'),
            array('username', 'match', 'pattern' => '/^(?!(\d|GW\d{8,10}))[\x{4e00}-\x{9fa5}A-Za-z0-9_\(\)（）]+$/u',
                'message' => Yii::t('member', '{attribute} 只能由中文、英文、数字及下划线组成,不能是纯数字、GW号'),
            ...
            array('username, salt, real_name, password2, email, street, identity_number', 'length', 'max' => 128),
...
//            array('identity_number', 'check_identity_number', 'on' => 'update_base'),
        	...
        	array('mobile,identity_type,identity_number,real_name,sex,identity_type', 'comext.validators.requiredExt', 'allowEmpty' => $this->status == self::STATUS_DELETE, 'on' => 'update'),	
            //快速注册
            array('username', 'unique', 'on' => 'quickRegister'),
            array('username', 'required', 'on' => 'quickRegister'),
            array('username', 'isCorrect', 'on' => 'quickRegister'),
            array('password', 'required', 'on' => 'quickRegister,updateBindEmail,bindEmail'),
            array('password', 'checkGaiAndPass', 'on' => 'upgrade'),
            array('email','required','on'=>'bindEmail,emailUnqiue'),
            array('email','comext.validators.emailUnique','on'=>'emailUnqiue,bindEmail'),
            array('sex,real_name,mobile','required','on'=>'editinfo')
        );
        if (!empty($this->rules)) {
     ...
     * @param $params
     */
    public function checkGaiAndPass($attribute, $params) {
        $session = Yii::app()->session;
        if (isset($session['tryLoginCount'])) {
            $count = $session['tryLoginCount'];
            if ($count > 10) {
                $this->addError('password', Yii::t('member', '输入错误次数过，请稍后关闭浏览器，重新打开！') . "<br />");
                return false;
            } else {
                $session['tryLoginCount'] += 1;
            }
        } else {
            $session['tryLoginCount'] = 1;
        }
        ...
        if ($gai_number) {
            if (!preg_match('/^GW[\d]{8}$/', $gai_number)) {
                $this->addError('gai_number', Yii::t('member', '您输入的盖网编号不正确，请重新输入！') . "<br />");
                return false;
            }
            if (!$this->password) {
                $this->addError('password', Yii::t('member', '密码不能为空!') . "<br />");
                return false;
            } elseif (strlen($this->password) < 6) {
                return false;
            }
            $model = Member::model()->find(array('condition' => 'gai_number=:gai_number', 'params' => array(':gai_number' => $gai_number)));
            if (!$model) {
                $this->addError('password', Yii::t('member', '盖网编号或者密码不正确！') . "<br />");
                return false;
            } else {
                if (!$model->mobile) {
                    $this->addError('errType', 'no_mobile');
                    $this->addError('gai_number', '你还未绑定手机号码，请登录并绑定手机号码后,再重新提交升级申请');
                } else {
            ...
            $this->addError('gai_number', Yii::t('member', '盖网编号不能为空！') . "<br />");
        }
    }

    //同步注册到盖讯通
    public static function getSynchronous($gaiNumber, $password, $nickname, $mobile, $passswordKey) {
        //post方式
        $ch = curl_init();
...
//        return json_decode($file_contents,true);
    ...
    //同步修改密码到盖讯通
    public static function getUpdatePassword($gaiNumber, $password, $passswordKey) {
        //post方式
        $ch = curl_init();
...
//        return json_decode($file_contents,true);
    ...
    public function isCorrect($attribute, $params) {
        if (is_numeric($this->username)) {
            $users = Yii::app()->db->createCommand()->select('gai_number')
                    ->from('{{member}}')->where('mobile=:m', array(':m' => $this->username))->queryAll();
            if (!empty($users)) {
                $this->addError($attribute, Yii::t('member', '此用户名已经被注册'));
            }
        }
    }
    ...
    public function checkPassword($attribute, $params) {
        $password = Yii::app()->db->createCommand()
            ...
            ->queryRow();
        if (!CPasswordHelper::verifyPassword($this->password . $password['salt'], $password['password'])) {
            $this->addError($attribute, Yii::t('member', '与登录密码不一致,请重新输入'));
        }
    }
    ...
    public function check_identity_number($attribute, $params) {
        ...
        if (!empty($this->identity_number) && !preg_match($preg, $this->identity_number)) {
...
//            'childoffline' => array(self::HAS_ONE, 'Member', 'pid', 'on' => 'childoffline.role = 1'),
    ...
    public function attributeLabels() {
        return array(
            ...
            'password2' => Yii::t('member', '积分管理密码'), //二级密码
            'password3' => Yii::t('member', '支付密码'), //三级密码
    ...
    public $member_id;
    public $promotion_id;
    public function searchPromotionMem() {
        $criteria = new CDbCriteria;
        $criteria->compare('t.gai_number', $this->gai_number, true);
        $criteria->compare('t.username', $this->username, true);
        $criteria->compare('t.mobile', $this->mobile, true);
        //连表查询 会员类型
        $criteria->select = 't.id,t.logins,mt.promotion_id,t.username,t.gai_number,t.mobile,t.register_time';
        $criteria->join = '  LEFT JOIN {{member_exten}} as mt on t.id = mt.member_id ';
        if($this->promotion_id) {
            $criteria->addCondition('mt.promotion_id='.$this->promotion_id);
        }  
        return new CActiveDataProvider($this, array(
                'criteria' => $criteria,
                'sort' => array(
     ...
     * @return array
     */
   public static function getPositionMemInfo($mid){
      $memInfo = Yii::app()->db->createCommand()
      ->select('m.id,m.gai_number,m.username,p.name,m.logins')
      ->from('{{member}} m')
      ->leftJoin('{{member_exten}} mt', 'm.id = mt.member_id')
      ->leftJoin('{{promotion_channels}} p', 'mt.promotion_id = p.id')
      ->where('m.id=:mid', array(':mid' =>$mid))
      ->queryRow();
      return $memInfo;
   }
...
//修改二级密码
    ...
    public function generateGaiNumber() {
        ...
        if ($this->exists('gai_number="GW' . $number . '"') || self::checkGoodNumberStatus($number)) {
	...
	/**
     * 个人资料完成程度百分比 V2.0改版,规则改变了
     * @return float
     */
    public function infoPercentV20() {
		
        $infoArr = array(
            empty($this->username) ? 0 : 1,//用户名
            empty($this->sex) ? 0 : 1,//性别
            empty($this->real_name) ? 0 : 1,//真实姓名
            empty($this->gai_number) ? 0 : 1,//盖网号
            empty($this->head_portrait) ? 0 : 1,//头像
            empty($this->birthday) ? 0 : 1,//生日
            empty($this->email) ? 0 : 1,//邮箱
            empty($this->mobile) ? 0 : 1,//手机
            empty($this->province_id) ? 0 : 1,//所在地区
            empty($this->street) ? 0 : 1,//详细地址
        );
        return intval(array_sum($infoArr) / count($infoArr) * 100);
    }
...
//        }
    ...
    public function getTotalFreezeCashNew() {
        ...
        //2016-01-29 增加获取type=5的冻结积分(拍卖活动)
        //$balance_cash = AccountBalance::getTodayAmountByGaiNumber($this->gai_number, AccountBalance::TYPE_FREEZE);
        //$history_cash = AccountBalanceHistory::getTodayAmountByGaiNumber($this->gai_number, AccountBalance::TYPE_FREEZE);
        //return $old_gai_cash * 1 + $new_gai_cash * 1 + $balance_cash*1 + $history_cash*1;
    ...
    /**
     * 检查是否需要输入验证码，当用户输入手机验证码错误超过3次,则显示图形验证码
     * @return bool
     */
    public static function showCaptcha() {
        $showCaptcha = Yii::app()->user->getState('showCaptcha');
        return $showCaptcha == 3 && CCaptcha::checkRequirements();
    }
    ...
    public function afterSave() {
        if (!$this->isNewRecord) {
            Tool::cache('memberLoginCache')->set($this->id, Member::model()->findByPk($this->id), 1800);
        }
    }

    /**
     * 获取用户的基本信息，用于oauth2 server
     * @param $id
     * @return mixed
     */
    public static function getUserInfoById($id) {
        $sql = 'SELECT
            ...
            FROM
                gw_member AS m
            LEFT JOIN gw_region AS r1 ON r1.id = m.province_id
            LEFT JOIN gw_region AS r2 ON r2.id = m.city_id where m.id=:id';
        return Yii::app()->db->createCommand($sql)->bindValue(':id', $id)->queryRow();
    }
     ...
     * @author LC
     */
    public static function getMemberCacheById($memberId)
    {
    	$member = Tool::cache('memberLoginCache')->get($memberId);
    	if(empty($member))
    	{
    		$member = Member::model()->findByPk($memberId);
    		Tool::cache('memberLoginCache')->set($memberId,$member,1800);
    	}
    	return $member;
    }

    /**
     * 获取用户的基本信息，用于检测用户手机绑定
     * @param $gaiNumber
     * @return mixed
     */
    public static function getUserInfoByGw($gaiNumber,$fields='mobile') {
        $result =  Yii::app()->db->createCommand()->select($fields)->from("{{member}}")->where('gai_number=:gai_number',array(':gai_number'=>$gaiNumber))->queryRow();
        return $result;
    }
    ...
    public static function getInfoById($id,$fields='*'){
        ...
        $result =  Yii::app()->db->createCommand()->select($fields)->from("{{member}}")->where('id=:id',array(':id'=>$id))->queryRow();
        return $result;   
    }
     ...
     * @return mixed
     */
    public static function checkUserPhoneById($id) {
        $result =  Yii::app()->db->createCommand()->select('mobile')->from("{{member}}")->where('id=:id',array(':id'=>$id))->queryRow();
        return $result;
    }
     ...
     * @param  $number
     */
    public static function checkGoodNumberStatus($number){
    	$data = Yii::app()->db->createCommand()->select('number')->from("gw_member_good_number")->where('number=:number',array(':number'=>$number))->queryRow();
    	if(!empty($data)){
    		return true;  
    	}else{
    		return false;
     ...
     * @param  $number
     */
    public static function updateGoodNumberStatus($number){
    	try {
    		$connection=Yii::app()->db;
    		$transaction = $connection->beginTransaction();
    		$time = time();
    		$sql = "INSERT INTO gw_member_good_number_bind_log(good_number,create_time) VALUES('".$number."','".$time."')";
    		$updatesql = "update ".MemberGoodNumber::model()->tableName()." SET status = '1' WHERE number = '".$number."'";
