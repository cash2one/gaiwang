...
class CashHistory extends CActiveRecord {
    ...
    const MONEY_ONE=1; //小于等于1万 不考虑白名单
    const MONEY_TWO=2; //大于1万 考虑白名单
    const MONEY_THREE=3; //小于等于3万 不考虑白名单
    const MONEY_FOUR=4; //大于3万 考虑白名单
    ...
    public static function MoneyShow() {
    	return array(
    			...
    			self::MONEY_ONE => Yii::t('cashHistory', '小于等于1万'),
    			self::MONEY_TWO => Yii::t('cashHistory', '大于1万'),
    			self::MONEY_THREE => Yii::t('cashHistory', '小于等于3万'),
    			self::MONEY_FOUR => Yii::t('cashHistory', '大于3万'),
    	);
    }
    ...
    public function rules() {
        return array(
            ...
            array('applyer, account_name, bank_name, bank_address,account, score, ratio, money,
            ...
            array('applyer, account_name, bank_name, bank_address, account, symbol', 'length', 'max' => 128),
            array('score, ratio, money, factorage, base_price', 'length', 'max' => 15),
            array('apply_time, ip, member_id', 'length', 'max' => 11),
            array('id, applyer, account_name, bank_name, bank_address, account, score, ratio, money,
			 apply_time, ip, status, reason, type, member_id, factorage, symbol, base_price,
			  end_score,end_time,province_id,city_id,district_id,mobile,order,offlineMoney,agentMoney', 'safe', 'on' => 'search'),
            array('reason,offlineMoney,agentMoney,role,gai_number,is_review,is_check,code','safe'),
        );
    }
    ...
    public function attributeLabels() {
        return array(
            ...
            'symbol' => Yii::t('cashHistory', 'unknow'),
            'base_price' => Yii::t('cashHistory', 'unknow'),
    ...
    /**
     * 后台兑现、提现搜索
     * @param int $type 类型 const TYPE_CASH = 1;  const TYPE_COMPANY_CASH = 2;（ 1兑现，2提现）
     * @param bool $export 是否导出excel
     * @return CActiveDataProvider|CDbCriteria
     */
    public function search($type,$export=false) {
        ...
        if (!empty($this->order)) {
        	...
        	$criteria->order ='t.apply_time asc';
         }
        $criteria->compare('account_name', $this->account_name, true);
...
//        var_dump($dateTime);exit;
        ...
        if($type==self::TYPE_CASH){
            $criteria->select = 't.*,gai_number as member_id,m.mobile';
            $criteria->join = 'left join {{member}} as m on m.id=t.member_id';
        }else{
            $criteria->compare('m.role',$this->role);
            $criteria->select = 't.*,m.mobile,m.gai_number,m.role';
            $criteria->join  = 'left join {{member}} as m on m.id=t.member_id';
//            $criteria->join .= ' left join {{enterprise}} as e on e.id=m.enterprise_id';
            //白名单修改
            $this->whiteList=explode(',', Tool::getConfig('cashhistory','whiteList'));
            //金额筛选
            if($this->moneyType==self::MONEY_ONE){
            	$criteria->compare('t.money', '<=' . 10000);
            }else if($this->moneyType==self::MONEY_TWO){
            	$criteria->compare('t.money', '>' . 10000);
            	$criteria->addInCondition('m.gai_number', $this->whiteList);
            }else if($this->moneyType==self::MONEY_THREE){
            	$criteria->compare('t.money', '<=' . 30000);
             }else if($this->moneyType==self::MONEY_FOUR){
            	$criteria->compare('t.money', '>' . 30000);
            	$criteria->addInCondition('m.gai_number', $this->whiteList);
            }
        }
	...
	public function searchList($type) {
        ...
        if($type==self::TYPE_CASH){
            ...
            $criteria->select = 't.*,mi.mobile,m.gai_number as member_id ';
