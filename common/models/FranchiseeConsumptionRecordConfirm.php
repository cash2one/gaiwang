...
/**
 * This is the model class for table "{{franchisee_consumption_record_confirm}}".
 *
 * The followings are the available columns in table '{{franchisee_consumption_record_confirm}}':
 * @property string $id
 ...
 */
class FranchiseeConsumptionRecordConfirm extends CActiveRecord
{
	...
	{
		return '{{franchisee_consumption_record_confirm}}';
	}
    ...
    const STATUS_PASS = 1;		//已同意
    const STATUS_REFUSE = 2;	//已拒绝
    const STATUS_AUDITI = 3 ; //已审核

    public static function getApplyStatus($key = null) {
        $data = array(
