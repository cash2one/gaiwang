...
class AppAdvert extends CActiveRecord {
    ...
    public function rules() {
        return array(
            array('name, code, type, status', 'required'),
            array('type, status, width, height', 'numerical', 'integerOnly' => true),
    ...
    public static function getAppAdvertType($key = false) {
        $type = array(
            self::TYPE_IMAGE => Yii::t('advert', '图片'),
            self::TYPE_TEXT => Yii::t('advert', '文字'),
            self::TYPE_SLIDE => Yii::t('advert', '幻灯')
        );
        if ($key === false)
            return $type;
        return $type[$key];
...
//        return array('INDEX_HEADER_SLIDE_LEFT', 'INDEX_SLIDE_UP', 'INDEX_SLIDE_LEFT', 'HOLTEL_SLIDE_LEFT');
    ...
    public static function generateConventionalAd($code = null) {
        ...
        $ap = Yii::app()->db->createCommand()->from('{{app_advert}} as t')->select('t.code,t.type,ap.*')
        ...
        if(!empty($ap)){
            foreach ($ap as $v) {
                $data[$v['code']][$v['group']][$v['seat']][$v['id']] = $v;
            }
