...
//        $config = unserialize(base64_decode(file_get_contents($file)));
    ...
    public static function requestParamsDispose($params) {
        $purifier = new CHtmlPurifier(); //过滤 xss
        $arr = array();
        if (is_array($params)) {
                ...
                if (is_array($v)) {
                    ...
                    foreach ($v as $sk => $sv) {
                        ...
                        if (in_array($qv, $svKey)) { // 如果请求参数合法，则取合法参数
                            $arr[$k] = $purifier->purify($qv);
                            break;
                        }
                    }
                } else {
                    $gp = Yii::app()->request->getParam($k, $v);
                    $arr[$k] = self::magicQuotes(htmlspecialchars($purifier->purify($gp)));
                }
            }
        }
        return $arr;
    ...
    public static function getPosition() {
        ...
        if ($cookie && !empty($cookie->value)) {
            ...
            Yii::import('comvendor.ip.IpTable');
...
//     	$string = strtolower($string);
    ...
    public static function floatcmp($f1, $f2, $precision = 10) {
        return bccomp($f1,$f2,$precision)==0 ? true: false;
    }
...
//            curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);    //是否抓取跳转后的页面
     ...
     */
    public static function getMemberLoginCache($member_id){
        $member = Tool::cache('memberLoginCache')->get($member_id);
        if(empty($member)){
            $member = Member::model()->findByPk($member_id);
            Tool::cache('memberLoginCache')->set($member_id,$member,50);
        }
        return $member;
    }
     ...
     * @author jiawei.liao
     */
    public static function array_set($array, $key) {
        $arr = array();
        $str = '';
        foreach ($array as $v) {
            $arr[] = $v[$key];
        }
        $str = count($arr) < 2 ? $arr[0] : implode(',',$arr);
        return $str;
    }
	
	
	/**
	* UTF-8字符截取函数,本类那个实现不了想要的效果
	* @param string $string 要截取的字符串
	* @param integer $length 要截取的长度 
	* @param string $dot 附加的显示字符
	* @param string $charset 默认的字符集
	* @return string 返回截取后的字符串
	* @author wenhao.li
	*/
	public static function dCutUtf8String($string, $length, $dot = ' ...', $charset = 'utf-8') {
    ...
    /*
     * 使用闪达邮件服务代发商发送邮件
     * @param string $address 发送网址
     * @param string $subject 主题
     * @param string $template_name 模板名称
     * @param array $value 替换变量(当使用模板发送用作替换变量时必须为二维数组，当使用普通发送用作邮件正文可以为字符串)
     * @param string $type 发送类型，1、模板，2、批量，3、普通
     * @author qinbao.deng
     */
    public static function sendCouldEmail($address, $subject, $template_name, $type, $value = null){
        $config = Tool::getConfig($name = 'email');

        $API_USER = 'gaiwang';
        $API_KEY = 'HgDldRbqRzeL4nQE';
        $url = 'http://sendcloud.sohu.com/webapi/mail.send_template.json';//默认使用模板接口

        $data = array(
            'api_user' => $API_USER,// 使用api_user和api_key进行验证
            'api_key' => $API_KEY,
            'from' => $config['fromMail'],//发信人
            'fromname' => $config['fromName'],
            'resp_email_id' => 'true',
        );

        //模板发送
        if($type == EmailLog::TEMPLATE_MAIL){
            $vars = json_encode( array('to' => array($address),
//                    "sub" => array("name" => Array('123456'),'url'=>array('https://www.baidu.com'))
                    'sub' => $value
                )
            );

//            $data['subject'] = $subject;
            $data['substitution_vars'] = $vars;
            $data['template_invoke_name'] = $template_name;//模板别称
        }

        //批量发送
        if($type == EmailLog::TEMPLATE_MAIL_LIST){
//            $data['subject'] = $subject;
            $data['use_maillist'] = 'true';
            $data['to'] = $address;//使用地址列表的别称地址
            $data['template_invoke_name'] = $template_name;//模板别称
        }

        //普通发送
        if($type == EmailLog::MAIL){
            $url = 'http://sendcloud.sohu.com/webapi/mail.send.json';
//            $data['subject'] = $subject;
//            $data['html'] = '测试！';//正文
            $data['to'] = $address;//收件人地址, 多个地址用';'分隔
            $data['html'] = $value;//正文
        }
        ...
        $rs = CJSON::decode($result);
        if(empty($rs)){
            $resultArr['send_status'] = EmailLog::STATUS_FAILD;
            $resultArr['create_time'] = time();
        }else{
            if($rs['message'] == 'success'){
                $resultArr['send_status'] = EmailLog::STATUS_SUCCESS;
                $resultArr['create_time'] = time();
            }else{
                $resultArr['send_status'] = EmailLog::STATUS_FAILD;
                $resultArr['create_time'] = time();
    ...
    /*按列名返回数组元素*/
    public static function returnArrayColomn($array = array(), $columnName = '')
    {
     ...
     * @param string  $s 中文
     * @param bool $isFirst 返回格式 [false:首字母|true:全拼音]
     * @param bool $ucFirst 首字母是否大写
     * @return string
     */
    public static function getPinyin($s, $isFirst = true,$ucFirst=false){
        static $pinYin;
        $s = trim($s);
        $len = strlen($s);
        if($len < 3) return $s;
        ...
        for($i = 0; $i < $len; $i++) {
            ...
            if($o < 0x80) {
                ...
                $z = $s[$i].$s[++$i].$s[++$i];
                if(isset($pinYin[$z])) {
                    $rs .= $isFirst ? $pinYin[$z][0] : ($ucFirst ? ucfirst($pinYin[$z]) : $pinYin[$z]);
                }else{
                    $rs .= '_';
                }
            }
