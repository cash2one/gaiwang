 ...
 *
 */
class GWRedisListTest {

    const DEFAULT_SEND_COUNT = 3;   //默认发送3次，如果3次都返回失败则不再发
    const GT_SMS_SEND_LISTS = 'gt_sms_send_lists';  //盖网通短信队列(消费的信息)
    const GT_SMS_SEND_LIST_CODES = 'gt_sms_send_list_codes';  //盖网通短信队列(验证码)
    const GT_EMAIL_SEND_LIST = 'gt_email_send_list';    //盖网通邮件队列
    ...
    public static function sendSmsGT(Array $arr) {
        ...
        if ($redisStatus == 1) {
            ...
            Yii::log('入队》'.'smsapi'.var_export($smsConfig,true).$arr);
            $sms = CJSON::encode($arr);
            self::set(self::GT_SMS_SEND_LISTS, $sms);          
        } else {
            self::send($arr);
        }
        return true;
    ...
    public static function sendSmsGTCode($arr) {
       $redisStatus = Tool::getConfig('smsmodel', 'isRedis');
        ...
        if ($redisStatus == 1) {
            $sms = CJSON::encode($arr);
            self::set(self::GT_SMS_SEND_LIST_CODES, $sms);
        } else {
           self::send($arr);
        }
        return true;
    ...
    public static function daemonSmsGT() {
        self::daemonSms(self::GT_SMS_SEND_LISTS);
    }
    ...
    public static function daemonSmsGTCode() {
        self::daemonSms(self::GT_SMS_SEND_LIST_CODES);
    }
...
//         self::delete($key);
    ...
    public static function send(&$info) {
        ...
        if ($rs['send_status'] == SmsLog::STATUS_FAILD && $info['count'] < self::DEFAULT_SEND_COUNT) {
            ...
            Yii::app()->db->createCommand()->update($tn, $updateArr, 'id=' . $info['id']);
        }
    }
...
//        $rs =Tool::sendEmail($info['email'], $info['subject'], $info['content'],$info['id']);
        $rs =Tool::sendCouldEmail($info['email'], $info['subject'], $info['template'], $info['sendType'], $info['value']);
