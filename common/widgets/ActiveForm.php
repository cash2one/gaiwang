...
class ActiveForm extends CActiveForm {
   ...
   private  function repeatSubmitJs($id){
     ...
     * @author zhenjun.xu
     */
    private  function repeatSubmitJs2($id){
        $cs = Yii::app()->clientScript;
        $layerUrl = Yii::app()->theme->baseUrl.'/js/layer/layer.js';
        $cs->registerScriptFile($layerUrl);
        $js = "js:function(form, data, hasError){
                if(!hasError){
                     layer.load(2);
                     return true;
        ...
        if(isset($this->clientOptions['afterValidate'])){
            return $this->clientOptions['afterValidate'];
        }else{
            return $js;
        }
    }
    ...
    public function run()
    {
        if(isset($this->htmlOptions['id']) && $this->enableRepeatSubmit == true){
            ...
            // 为afterValidate 赋值
            $this->clientOptions['afterValidate'] = Yii::app()->theme ? $this->repeatSubmitJs2($id) : $this->repeatSubmitJs($id);
        }
