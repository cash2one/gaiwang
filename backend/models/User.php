...
class User extends CActiveRecord {
    ...
    public function rules() {
        return array(
            ...
            array('password', 'comext.validators.passwordStrength', 'message' => Yii::t('user', '密码强度不够, 必须是字母加数字组成'), 'on' => 'create,modify'),
            array('status', 'in', 'range' => array(0, 1)),
            array('sex', 'in', 'range' => array(1, 2)),
            array('mobile', 'required', 'on' => 'update'),
            array('mobile', 'comext.validators.isMobile', 'errMsg' => Yii::t('user', '请输入正确的手机号码'), 'on' => 'create, update'),
            array('email', 'email'),
            array('logins', 'safe'),
    ...
    {
        return array(
            'assigments'   => array(self::BELONGS_TO,'AuthAssignment', array('id' => 'userid'),'joinType'=>'JOIN'),
        );
    }
        ...
        $criteria->with = array(
            'assigments' => array(
                ...
                'with' => array(
                    'role' => array(
                        'select' => 'name',
                        'with' => array('authItemChild'=>array('select'=>"child,parent"))
                    )
                ),
            ),
        );
        $criteria->select = 't.username';
        $authName = empty($_GET['authName']) ? 'Main.UserInfo' : $_GET['authName'];
        $criteria->compare('authItemChild.child',$authName);
        $pagination = array(
          'pageSize' => 200,
        );
        return new CActiveDataProvider($this, array(
     ...
     * 得到登陆者的名称
     */    
    public static function getAuthorNameBy($id){
    	$nameArr=User::model()->findByPk($id);
    	return isset($nameArr->username) ? $nameArr->username : '';
