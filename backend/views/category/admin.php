...
<?php if ($this->getUser()->checkAccess('Category.Create')): ?>
    <a class="regm-sub" href="<?php echo $this->createAbsoluteUrl('/category/create') ?>"><?php echo Yii::t('category', '添加分类') ?></a>
<?php endif; ?>
<?php if ($this->getUser()->checkAccess('Category.GenerateAllCategoryCache')): ?>
    <a class="regm-sub" href="<?php echo $this->createAbsoluteUrl('/category/generateAllCategoryCache') ?>"><?php echo Yii::t('category', '更新分类缓存') ?></a>
<?php endif; ?>

<?php
$operateLinks = '';
if ($this->getUser()->checkAccess('Category.Update')):
    $operateLinks .= "<a href='" . urldecode($this->createAbsoluteUrl('category/update', array('id' => '"+value+"'))) . "'>【编辑】</a>";
endif;
if ($this->getUser()->checkAccess('Category.Create')):
    $operateLinks .= "<a href='" . urldecode($this->createAbsoluteUrl('category/create', array('parentId' => '"+value+"'))) . "'>【添加子类别】</a>";
endif;
if ($this->getUser()->checkAccess('Category.Delete')):
    ...
    jQuery(function($) {
        $('#treeGrid').treegrid({
            ...
            columns: [[
                    ...
                    {field: 'id', title: '<?php echo Yii::t('category', '操作'); ?>', width: '300', formatter: function(value) {
