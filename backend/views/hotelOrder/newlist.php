...
<script type="text/javascript">
    function confirmCancle(orderid) {
        artDialog.confirm('<?php echo Yii::t('hotelOrder', '你确认要取消此订单吗？') ?>', function() {
            jQuery.ajax({
                type: 'POST',
                url: '<?php echo urldecode(Yii::app()->createAbsoluteUrl("hotelOrder/cancelNewOrder", array("id" => '\'+orderid+\''))); ?>',
                data: {
    ...
    'columns' => array(
        ...
        array(
            ...
            'buttons' => array(
                ...
                'verify' => array(
                    'label' => Yii::t('hotelOrder', '签收'),
                    'url' => 'Yii::app()->createAbsoluteUrl("hotelOrder/sign", array("id"=>$data->primaryKey))',
                    'imageUrl' => false,
                    'visible' => '($data->pay_status == HotelOrder::PAY_STATUS_YES) && Yii::app()->user->checkAccess("HotelOrder.Sign")'
                ),
                'cancel' => array(
                    'label' => Yii::t('hotelOrder', '取消'),
                    'url' => '"javascript:confirmCancle(\"$data->primaryKey\")"',
                    'visible' => "Yii::app()->user->checkAccess('HotelOrder.CancelNewOrder')",
                    'imageUrl' => false,
                ),
            )
        ),
