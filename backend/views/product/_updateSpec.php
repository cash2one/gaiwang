        ...
        <dd <?php echo $v['type'] == Spec::TYPE_IMG ? 'data-type="sp_group_val"' : ''; ?> >
            <ul class="spec">
                <?php foreach ($v['spec_value_data'] as $spv): ?>
                    <li>
                        <span class="spec_checkbox">
                            <?php echo CHtml::checkBox('spec_val[' . $v['id'] . '][' . $spv['id'] . ']', $spv['checked'],
                        ...
                        <span class="spec_name"><?php
                            if($spv['checked']){
                                ...
                                echo CHtml::textField('',$spv['name'],array('style'=>'style="border:solid 1px #ccc;"','maxlength'=>20));
                            }
                            ?></span>
                    </li>
                <?php endforeach; ?>
                ...
                <?php if ($v['type'] == Spec::TYPE_IMG): ?>
                    <table class="spec_table" style="display:none;" id="col_img_table">
                        <thead>
                        <tr>
                            <th><?php echo Yii::t('product', '颜色'); ?></th>
                            <th class="w250"><?php echo Yii::t('product', '图片（无图片可不填）'); ?></th>
                            <th><?php echo Yii::t('product','已有图片') ?></th>
                        </tr>
                        </thead>
                        <tbody>
                        ...
                        <tr>
                            <td colspan="3">
                                <p class="hint" style="padding-left:10px;">
                                    <?php echo Yii::t('product', '支持jpg、jpeg、gif、png格式图片。'); ?><br/>
                                    <?php echo Yii::t('product', '建议上传尺寸310x310、大小1.00M内的图片。'); ?><br/>
                                    <?php echo Yii::t('product', '商品详细页选中颜色图片后，颜色图片将会在商品展示图区域展示。'); ?>
                                </p>
                            </td>
                        </tr>
                        </tbody>
...
<dl id="stock_setting" class="spec-bg" <?php echo empty($goodsSpec['spec_value_array']) ? 'style="display:none"':'' ?> >
    <dt><?php echo Yii::t('product', '库存配置'); ?>：</dt>
    <dd class="spec-dd">
        <table border="0" cellpadding="0" cellspacing="0" class="spec_table">
            ...
            <th><span class="red">*</span><?php echo Yii::t('product', '库存'); ?></th>
            <th><?php echo Yii::t('product', '商品货号'); ?></th>
            </thead>
            <tbody id="spec_table">
