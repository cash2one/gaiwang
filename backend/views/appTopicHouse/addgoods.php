...
?>
<div class="ctx">
    <?php  $this->renderPartial('_searchgoods', array('model' => $AppHouseModel,'Hid'=>$Appmodel->id)); ?>
    <div>&nbsp;</div>
    ...
    <div id="dListTable" class="ctxTable">
        ...
        $this->widget('GridView',array(
            ...
            'dataProvider' => $model->searchAppTopic($Appmodel->id),
...
<script>
    ...
    var getRecommend = function() {
        ...
        var houseId = <?php echo $Appmodel->id;?>;
        var data = new Array();
        var id = '';
        ...
        if (data.length > 0) {
            $.post('<?php echo CHtml::normalizeUrl(array('/appTopicHouse/ajaxAddGoods')); ?>', {'ids[]': data, 'houseId':houseId,'YII_CSRF_TOKEN': '<?php echo Yii::app()->request->csrfToken; ?>'}, function(data) {
                    if(data){
                        if(data != 1 && data != 0){
                        	 alert('Id为'+data+'的商品已参加其他活动，与本仕品馆互斥');  
                        }else{
                        	 alert('添加成功!');  
                        }
