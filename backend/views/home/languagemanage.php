...
<?php //生成英文语言包
if(strpos($messagesConfig['messagePath'], 'front') !== FALSE){
    if(Yii::app()->user->checkAccess('Home.LanguageFrontend')){
        $sendUrl = 'createFrontPackFromDb';
        echo CHtml::link('生成英文包', '', array('onclick' => 'createPackage()', "class" => "regm-sub",'style'=>'float:right'));
    }
}elseif(strpos($messagesConfig['messagePath'], 'backend') !== FALSE){
    if(Yii::app()->user->checkAccess('Home.LanguageBackend')){
        $sendUrl = 'createBackPackFromDb';
        echo CHtml::link('生成英文包', '', array('onclick' => 'createPackage()', "class" => "regm-sub",'style'=>'float:right'));
    }
}elseif(strpos($messagesConfig['messagePath'], 'api') !== FALSE){
    if(Yii::app()->user->checkAccess('Home.LanguageApi')){
