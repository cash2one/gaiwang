...
<?php $this->renderPartial('_searchcash', array('model' => $model)); ?>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab-reg2">
        ...
        <tr class="tab-reg-title">
            ...
            <th>id</th>
            <th>
                <?php echo Yii::t('cashHistory', '普通会员'); ?>
            </th>
            <th>
        ...
        <?php foreach ($log as $v): ?>
            <tr class="info">
                ...
                <td>
                    <?php if(Yii::app()->user->checkAccess('CashHistory.MemberSetReview')): ?>
                        <?php echo $v::showReviewStatus($v->id,$v->is_review,'memberSetReview') ?>
                    <?php endif; ?>
                </td>
                <td>
                    <?php if (Yii::app()->user->checkAccess('CashHistory.MemberApplyCashDetail')): ?>
                        <?php echo CHtml::link(Yii::t('cashHistory', '【查看】'), $this->createUrl('cashHistory/memberApplyCashDetail', array('id' => $v->id))); ?>
                    <?php endif; ?>
                </td>
            </tr>
            <tr class="user">
        ...
        <tr>
            <td colspan="11" align="center">
                <?php if (Yii::app()->user->checkAccess('CashHistory.MemberCheckedBatch')): ?>
                    <?php echo CHtml::button(Yii::t('cashHistory', '批量审核申请'), array('data-status' => CashHistory::CHECK_YES, 'class' => 'regm-sub checkedBatch')) ?>
                <?php endif; ?>
                <?php if (Yii::app()->user->checkAccess('CashHistory.MemberCashBatchUpdate')): ?>
            ...
            if (ids.length == 0) {
                ...
                art.dialog({
                    ...
                    ok: function() {
                        $.ajax({
                            type: "POST",
                            url: "<?php echo $this->createUrl('cashHistory/memberCheckedBatch') ?>",
                            data: {
                                idArr: ids.join(','),
            ...
            if (ids.length == 0) {
                ...
                art.dialog({
                    ...
                    ok: function() {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "<?php echo $this->createUrl('cashHistory/memberCashBatchUpdate') ?>",
                            data: {
                                idArr: ids.join(','),
                                reason: reason,
                                YII_CSRF_TOKEN: '<?php echo Yii::app()->request->csrfToken ?>',
