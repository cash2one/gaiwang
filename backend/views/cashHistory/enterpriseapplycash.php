...
<?php $this->renderPartial('_searchcash', array('model' => $model)); ?>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab-reg2">
        ...
        <tr class="tab-reg-title">
            ...
            <th>id</th>
            <th>
                <?php echo Yii::t('cashHistory', '申请企业会员'); ?>
            </th>
            <th>
                <?php echo Yii::t('cashHistory', '盖网编号'); ?>
            </th>
            <th>
                <?php echo Yii::t('cashHistory', '类型'); ?>
            </th>
            <th>
            ...
            <th>
                <?php echo Yii::t('cashHistory', '申请提现金额'); ?>
            </th>
            <th>
            ...
            <th>
                <?php echo Yii::t('cashHistory', '实扣金额'); ?>
            </th>
            <th>
            ...
            <tr class="info">
                <td>
                    <?php if (in_array($v->status, array($v::STATUS_APPLYING,$v::STATUS_CHECKED,$v::STATUS_TRANSFERING)) || $v->is_check==$v::CHECK_NO ): ?>
                        <?php echo CHtml::checkBox('CacheKey',false,array('value'=>$v->id,'data-status'=>$v->status,'data-check'=>$v->is_check,'data-money'=>$v->money,'data-member'=>$v->member_id)) ?>
                ...
                <td>
                    <?php echo CashHistory::memberCashType($v->role); ?>
                </td>
                <td>
                ...
                <td>
                    <?php if(Yii::app()->user->checkAccess('CashHistory.EnterpriseSetReview')): ?>
                        <?php echo $v::showReviewStatus($v->id,$v->is_review,'enterpriseSetReview') ?>
                    <?php endif; ?>
                </td>
                <td>
                    <?php if (Yii::app()->user->checkAccess('CashHistory.EnterpriseApplyCashDetail')): ?>
                        <?php echo CHtml::link(Yii::t('cashHistory', '【查看】'), $this->createUrl('cashHistory/enterpriseApplyCashDetail', array('id' => $v->id))); ?>
                    <?php endif; ?>
                </td>
            </tr>
            <tr class="user">
                <td colspan="14">
                    ...
                    <b class="b-neme"><?php echo Yii::t('cashHistory', '商家名称'); ?>:</b>
        ...
        <tr>
            ...
            <?php if (Yii::app()->user->checkAccess('PaymentBatch.EnterpriseCheckedDfBatch')): ?>
              <span style="color: #FF3C3C;font-size:14px;font-weight:bold"><?php echo Yii::t('cashHistory', '批次号')?>:</span>
              <input class="text-input-bj  least" type="text" value="<?php echo $this->getCookie('batchNum');?>" name="batchId" id="batchId" maxlength="128">
              <?php echo CHtml::button(Yii::t('cashHistory', '批量代付申请'), array('data-status' => CashHistory::CHECK_YES, 'class' => 'regm-sub checkedDfBatch')) ?>
            <?php endif; ?>
                
                <?php if (Yii::app()->user->checkAccess('CashHistory.EnterpriseCheckedBatch')): ?>
                    <?php echo CHtml::button(Yii::t('cashHistory', '批量审核申请'), array('data-status' => CashHistory::CHECK_YES, 'class' => 'regm-sub checkedBatch')) ?>
                <?php endif; ?>
                <?php if (Yii::app()->user->checkAccess('CashHistory.EnterpriseCashBatchUpdate')): ?>
    ...
    <script>
        ...
        //批量审核代付操作
        $("input.checkedDfBatch").click(function(){
            var ids = [];
            var details = [];
            var money=[];
            var batchNum=$("#batchId").val();
            ...
            if (ids.length == 0) {
                art.dialog({
                    icon: 'error',
                    content: '<?php echo Yii::t('cashHistory', '请选择符合条件的提现记录'); ?>',
                    lock: true
                });
            }else{
                ...
                art.dialog({
                    ...
                    ok: function(){
                        $.ajax({
                            type: "POST",
                            url: "<?php echo $this->createUrl('PaymentBatch/enterpriseCheckedDfBatch') ?>",
                            data: {
                                ids: ids.join(','),
                                num: batchNum,
            ...
            if (ids.length == 0) {
                ...
                art.dialog({
                    ...
                    ok: function() {
                        $.ajax({
                            type: "POST",
                            url: "<?php echo $this->createUrl('cashHistory/enterpriseCheckedBatch') ?>",
                            data: {
                                idArr: ids.join(','), 
                                YII_CSRF_TOKEN: '<?php echo Yii::app()->request->csrfToken ?>',
                                check:status
        ...
        $("input.updateApply").click(function() {
            ...
            if (ids.length == 0) {
                art.dialog({
                    icon: 'error',
                    content: '<?php echo Yii::t('cashHistory', '请选择符合条件的提现记录,批量转账需要先操作：批量审核申请'); ?>',
                    lock: true
                });
            } else {
                var updateTitle = $(this).val();
                ...
                art.dialog({
                    ...
                    ok: function() {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "<?php echo $this->createUrl('cashHistory/enterpriseCashBatchUpdate') ?>",
                            data: {
                                idArr: ids.join(','),
                                reason: reason,
                                YII_CSRF_TOKEN: '<?php echo Yii::app()->request->csrfToken ?>',
