...
<?php $this->renderPartial('_searchcash', array('model' => $model)); ?>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab-reg2">
        ...
        <tr class="tab-reg-title">
            ...
            <th>id</th>
            <th>
                <?php echo Yii::t('cashHistory', '申请会员'); ?>
            </th>
            <th>
            ...
            <th>
                <?php echo Yii::t('cashHistory', '兑现比率'); ?>
            </th>
            <th>
                <?php echo Yii::t('cashHistory', '手续费(积分)'); ?>
            </th>
            <th>
                <?php echo Yii::t('cashHistory', '手续费率'); ?>
            </th>
            <th>
                <?php echo Yii::t('cashHistory', '实扣积分'); ?>
            </th>
            <th>
        ...
        <?php foreach ($log as $v): ?>
            <tr class="info">
                ...
                <td>
                    <span class="jf"><?php echo sprintf('%0.2f', $v->factorage * $v->money / 100 / $v->ratio); ?></span>
                </td>
                <td>
                ...
                <td>
                    <?php if(Yii::app()->user->checkAccess('CashHistory.SetReview')): ?>
                        <?php echo $v::showReviewStatus($v->id,$v->is_review) ?>
                    <?php endif; ?>
                </td>
                <td>
                    <?php if (Yii::app()->user->checkAccess('CashHistory.ApplyCashDetail')): ?>
        ...
        <tr>
            ...
            <?php endif; ?>
                ...
                <?php if (Yii::app()->user->checkAccess('CashHistory.CheckedBatch')): ?>
                    <?php echo CHtml::button(Yii::t('cashHistory', '批量审核申请'), array('data-status' => CashHistory::CHECK_YES, 'class' => 'regm-sub checkedBatch')) ?>
                <?php endif; ?>

                <?php if (Yii::app()->user->checkAccess('CashHistory.CashBatchUpdate')): ?>
