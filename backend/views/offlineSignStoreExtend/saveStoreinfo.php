...
$this->breadcrumbs = array(
    $breadcrumbsMap[$apply_type]['breadcrumbs'],
);
?>
<?php $form=$this->beginWidget('CActiveForm', array(
    'id'=>'offline-sign-store-extend-form',
    'action' => '/?r=offline-sign-store-extend/save-old-franchisee&id='.$storeInfoModel->id.'&role='.$role.'&num='.$storeNum,
...
<div class="main">
    ...
    <div class="sign-conten">
        ...
        <div class="sign-list">
            <ul>
                ...
                <li>
                    ...
                    <?php echo $form->dropDownList($model,'install_area_id',Region::getFranchiseeArea($extendModel->agent_id),array(
                        'class' => 'sign-select',
                        'prompt' => Yii::t('Public','选择大区'),
                        'ajax' => array(
                            'type' => 'POST',
                            'url' => $this->createUrl('region/updateAreaProvince'),
                            'dataType' => 'json',
                            'data' => array(
        ...
        <div class="sign-btn">
            <input type="submit" value="保存" id="btnSignBack-s"
                   class="btnSignSubmit"/>
            <input type="button" value="返回" onclick="localHref()" id="btnSignBack" class="btnSignBack"/>
        </div>
    </div>
</div>
    <?php $this->endWidget(); ?>
...
<script>
    function localHref(){
        window.location.href = "<?php echo Yii::app()->controller->createUrl("OfflineSignStoreExtend/update", array("id"=>$extendId,"role"=>$role))?>";
    }
        /**
         * 红色显示错误字段
         */
...
<script>
    $(document).ready(function(){
        ...
        var memDis = $("#OfflineSignStore_member_discount");
...
<style>
    ...
    div.errorMessage{float: left;position:relative;}
    select{float: left;}
</style>