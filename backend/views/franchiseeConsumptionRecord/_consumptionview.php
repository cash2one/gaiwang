<tr class="info">
	<td>
		<?php if($data->status == FranchiseeConsumptionRecordConfirm::STATUS_APPLY || $data->status == FranchiseeConsumptionRecordConfirm::STATUS_AUDITI):?>
...
<!--		<input type="hidden" value="<?php //echo $data->fcrid?>"/>-->
    ...
    <td>
        <span style="color:<?php  echo $data->status== FranchiseeConsumptionRecordConfirm::STATUS_PASS? 'green' : 'red';?>">
    		<?php echo FranchiseeConsumptionRecordConfirm::getApplyStatus($data->status)?>
    	</span>
    </td>
    <td>
    ...
    <td>
        <?php if($data->status != FranchiseeConsumptionRecordConfirm::STATUS_APPLY):?>
        <a class='regm-sub' href='javascript:do_info(<?php echo $data->id ?>,<?php echo $data->status?>,"confirm")'>审核记录</a>
        <?php endif;?>
    </td>
</tr>
<tr class="user">
