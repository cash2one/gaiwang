...
?>
<?php $this->renderPartial('_create_'.$model->categoryId, array('model' => $model, 'labels'=>$labels)); ?>
...
function checkCreateForm(){
	...
	var length      = $('#SeckillRulesSeting_product_category_id').find("input[type='checkbox']:checked").length;
	...
	if(length<1){
		$('#SeckillRulesSeting_product_category_id_em_').html('参加商品类别 请选择商品类别');
		$('#SeckillRulesSeting_product_category_id_em_').css('display', '');
		return false;
	}else{
		$('#SeckillRulesSeting_product_category_id_em_').css('display', 'none');
	}
	...
	if(categoryId == 1){
		...
		if(discount == 1){
			...
			if(discountPrice<0.01 || discountPrice>1000){
				$('#SeckillRulesSeting_discount_price_em_').html('限定价格 请填写0.01-1000的数,小数点后保留两位');
	...
	$.ajax({
		...
		type: 'post',
		dataType: "json",
		data:{'post':1, 'startTime':startTime, 'endTime':endTime, 'singupStart':singupStart, 'singupEnd':singupEnd, 'name':name, 'seckillSort':seckillSort, 'categoryId':categoryId, 'rulesId':rulesId,'limitNum':limitNum,'buyLimit':buyLimit,'sellerLimit':sellerLimit},
		error: function(){},
