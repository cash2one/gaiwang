...
<?php if (Yii::app()->user->checkAccess('Advert.Create')): ?>
    <input id="Btn_Add" type="button" value="<?php echo Yii::t('advert', '创建广告位'); ?>" class="regm-sub" onclick="location.href = '<?php echo $this->createAbsoluteUrl("/advert/create"); ?>'" />
<?php endif; ?>
...
$this->widget('GridView', array(
    ...
    'columns' => array(
        ...
        array(
            ...
            'value' => '$data->direction != Advert::TYPE_GOODS ? (" ".CHtml::link( $data->pictureCount, array("/advertPicture/admin","aid"=>"$data->id") ,array("class"=>"reg-sub" ))):" 无 "',
            'visible' => "Yii::app()->user->checkAccess('AdvertPicture.Admin')",
        ),
        array(
            'type' => 'raw',
            'name' => Yii::t('advert', '正在投放商品'),
            'value' => '($data->direction == 5 || $data->direction == 11) && $data->type == Advert::TYPE_GOODS ? (" ".CHtml::link( $data->goodsCount, array("/advertGoods/admin","aid"=>"$data->id") ,array("class"=>"reg-sub" ))):" 无 "',
            'visible' => "Yii::app()->user->checkAccess('AdvertGoods.Admin')",
        ),
        array(
            'class' => 'CButtonColumn',
