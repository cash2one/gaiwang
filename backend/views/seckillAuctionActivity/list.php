...
<?php
    if ($this->getUser()->checkAccess('SeckillAuctionActivity.SeckillAuctionCreate')) {
        ?>
        <a style=" margin-left:5px; float:left;" class="regm-sub"
           href="<?php echo Yii::app()->createAbsoluteUrl('/SeckillAuctionActivity/SeckillAuctionCreate/category_id/' . $model->categoryId) ?>">新建拍卖活动</a>&nbsp;&nbsp;
        <span
            style="float: left;margin-left: 25px;line-height:27px;font-weight: 700;">正在进行的活动数： <?php
            echo $model->getActivityNumber($model->categoryId); ?>
            个</span>
    <?php } ?>
...
$this->rPartial($formpage, array('model' => $model, 'dataProvider' => $dataProvider, 'labels' => $labels, 'pages' => $pages));
*/
$this->renderPartial('activity', array('model' => $model, 'dataProvider' => $dataProvider, 'labels' => $labels, 'pages' => $pages));
?>
...
<div id="pop-div" class="pop-box">
    ...
    <div class="pop-box-body" style="width:450px;">
        <ul id="vc"></ul>
        <ul id="yzm">请输入验证码：<input type="text" id="vcode" value="" maxlength="4" style="width:60px;"/><span
                id="vnum"></span></ul>
...
<div id="date-div" class="pop-box">
    ...
    <div class="pop-box-body" style="width:450px;">
        <ul style="height:40px; line-height:40px;"><span class="spname"><font color="#FF0000">*</font>拍卖活动名称: </span>
            <input type="text" id="SeckillRulesSeting_name" name="SeckillRulesSeting[name]"
                   class="text-input-bj  middle" placeholder="请填写活动的名称" value="" maxlength="10"/></ul>
...
<script language="javascript">
    ...
    function setActivityStatus(id, v) {
        ...
        if (v) {//结束活动
            postUrl = '<?php echo Yii::app()->createUrl('/SeckillAuctionActivity/stop');?>';
        } else {//正在进行
            postUrl = '<?php echo Yii::app()->createUrl('/SeckillAuctionActivity/start');?>';
        }
    ...
    function createDate() {
        ...
        $.ajax({
            url: '<?php echo Yii::app()->createUrl('/SeckillAuctionActivity/createDate');?>',
            type: 'POST',
            data: {
                'name': name,
                ...
                'singup_start_time': singupStart,
                'singup_end_time': singupEnd
            },
            dataType: 'json',
            timeout: 1000,
            error: function () {
    ...
    function changeStatus(v, rules_id) {
        window.location.href = "<?php echo Yii::app()->createUrl('/SeckillAuctionActivity/SeckillAuctionAdmin');?>&category_id=<?php echo $model->categoryId;?>&status=" + v + '&rules_id=' + rules_id;
    }
</script>
