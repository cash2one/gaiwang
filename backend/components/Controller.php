...
class Controller extends RController {
    ...
    private function _checkLogin($action) {
        ...
        return in_array($this->id . '/' . $action->id, $nonLogin) || Yii::app()->user->loginRequired();
    }
    ...
    public function getParam($name, $defaultValue = null, $filter = true) {
        $value = Yii::app()->request->getParam($name, $defaultValue);
        if (!$filter)
            return $value;
        return $this->magicQuotes($value);
    }
    ...
    public function checkAccess($operation) {
        return $this->getUser()->checkAccess($operation);
    }
    ...
    public function getMenu($type) {
        $mens = include(Yii::getPathOfAlias('application') . DS . 'config' . DS . 'menu.php');
        return $mens[$type];
    }
